...
def average_vertical_stress_increment_under_concentrated_load(P, x, y, z1, z2, zInc= 0.2):
...
def vertical_stress_increment_under_rectangular_loaded_area(q, B, L, x, y, z, eSize= 0.1):
    ...
    for i in range(0,nDivX):
        for j in range(0,nDivY):
            retval+= boussinesq.vertical_stress_increment_under_concentrated_load(P, x-xLoad, y-yLoad, z)
            yLoad+= eSize
        xLoad+= eSize
        yLoad= -L/2.0+eSize/2.0 # from square one.
    return retval

def average_vertical_stress_increment_under_rectangular_loaded_area(q, B, L, x, y, z1, z2, eSize= 0.1, zInc= 0.2):

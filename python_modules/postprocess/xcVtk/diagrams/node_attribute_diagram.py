...
import geom
from postprocess.xcVtk.diagrams import attribute_diagram as pd
from misc_utils import log_messages as lmsg

class NodeAttributeDiagram(pd.AttributeDiagram):
    ...
    def __init__(self, scaleFactor, lRefModSize, fUnitConv, sets, attributeName, defaultDirection= 'J', defaultValue= 0.0, rgMinMax= None):
        ...
        '''
        super(NodeAttributeDiagram,self).__init__(scaleFactor= scaleFactor, lRefModSize= lRefModSize, fUnitConv= fUnitConv, sets= sets, attributeName= attributeName, rgMinMax= rgMinMax)
        self.defaultDirection= defaultDirection
        self.defaultValue= defaultValue

    def getValueForNode(self, node):
        ''' Get the value of the attribute in the given node.

        :param node: node to get the attribute from.
        '''
        retval= None
        if(hasattr(node, self.featureName)):
            retval= getattr(node, self.featureName)
        if(retval is None):

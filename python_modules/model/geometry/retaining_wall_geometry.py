...
class CantileverRetainingWallGeometry(object):
  ...
  def writeGeometry(self,outputFile):
    ...
    outputFile.write("$b_{couronn}= "+fmt.Lengths.format(self.stemTopWidth)+"\\ m$\\\\\n")
    outputFile.write("Hauteur voile: \\\\\n")
    outputFile.write("$h_{voile}= "+fmt.Lengths.format(self.stemHeight)+"\\ m$\\\\\n")
    outputFile.write("Épaisseur encastrement: \\\\\n")
    outputFile.write("$b_{encast.}= "+fmt.Lengths.format(self.stemBottomWidth)+"\\ m$\\\\\n")
    outputFile.write("Épaisseur semelle: \\\\\n")
    outputFile.write("$b_{semelle.}= "+fmt.Lengths.format(self.footingThickness)+"\\ m$\\\\\n")

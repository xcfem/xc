...
fNameIn= "arcelor_uc_profiles.xlsx"
...
def post_process(shapesDict):
    ...
    for shapeName in shapesDict:
        ...
        for key in shapeRecord:
            if(key=='nmb'): # remove spaces in the name.
                value= shapeRecord[key].replace(' ','')
                shapeRecord[key]= value
    return shapesDict

shapesDict= su.read_shapes_from_spreadsheet(namePrefix= 'UC',columnOrder= columnOrder, columnUnits= columnUnits, spreadsheetFileName= fNameIn, E= 210000e6, nu= 0.3, post_processing_function= post_process)

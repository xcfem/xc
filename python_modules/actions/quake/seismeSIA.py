...
def S(soilClass):
    '''parameter to obtain the elastic response spectrum according SIA 261
       2014 (16.2 tableau 24).

    :param soilClass: soil class.
    '''
    retval= 1.4
    if(soilClass=='A'):
        retval= 1.0
...
def Tb(soilClass):
    ''' wave pertiod Tb to obtain the elastic response spectrum according to
    SIA 261 2003 (16.2 table 25).
...
def Tc(soilClass):
    '''wave period Tc to obtain the elastic response spectrum.
    according SIA 261 2003 (16.2 table 25).
...
def elasticReponseSpectrum(soilClass,viscousDamping,accelTerrain,T):
    ...
    :param soilClass: soil class.
    :param viscousDamping: viscous damping.
    :param accelTerrain: ground acceleration.
    :param T: period.
    '''
    s= S(soilClass)
...
def gammaF(CO):
    '''importance factor for buildings and bridges according to
       SIA 261 2003 (16.3.2 tableau 26).
...
def designSpectrum(soilClass,accelTerrain,CO,T,q):
    '''design spectrum for elastic response analysis according to 
       SIA 261 2014 (16.2.4.1).
...
def alpha(soilClass):
    '''
    alpha correction factor according SIA 261 2003 (16.4.4).
...
def grounDisplacement(soilClass,accelTerrain,CO):
    ''' design ground displacement according SIA 261 2003 (16.2.4.3).
...
def bLowerLimitAppuiFixe(soilClass,accelTerrain,CO,a12,l):
    '''Lower limit of bridge support dimension.
...
def bUpperLimitAppuiFixe(soilClass,accelTerrain,CO,a12,l):
    '''Upper limit of bridge support dimension.
...
def b1LowerLimitAppuiFixe(soilClass,accelTerrain,CO,a2,l):
    '''
    minimal dimension for b1 in bridge support according to
    SIA 261 2003 (16.4.4 et Figure 15).

...
class IsolatedFooting(object):
  def __init__(self,B,Nk=10e3,Sadm=1e3,FiArmPilar=12e-3,Lpilar=0.5):
    ''' Constructor.
  ...
  def getLaTeXHeader(self):
    ...
    retval+= "\\item Axil característico del pilar N$_k$ = " + "%5.2f".format(self.Nk) + " kN\n"
    retval+= "\\item Tensión admisible del terreno $\\sigma_{adm}$ = " + self.Sadm + " kN/m$^2$\n"
    retval+= "\\item Diámetro de la reinforcement longitudinal del pilar $\\Phi$ = " + "%2.1f".format(self.FiArmPilar) + " cm\n"
    retval+= "\item Escuadría del pilar $l$ = " +  "%3.2f".format(self.Lpilar) + " m\n"
    retval+= "\\end{itemize}\n"
    return retval
  ...
  def getLaTeXResults(self):
    ...
    retval+= "$$A=a^2=\\cfrac{N_k}{\\sigma_{adm}}=",(self.Nk/self.Sadm),"\\ m^ 2 \\rightarrow a =" + "%3.2f".format(self.Lzapata),"\\ m$$\n"
    retval+= "Canto de la zapata\n"
    vuelo=((math.sqrt(self.Nk/self.Sadm)-self.Lpilar)/2)
    retval+= "$$ vuelo = v = \\frac{a-l}{2} = " + "%3.2f".format(vuelo),"\\ m $$\n"
    aux=((15*self.FiArmPilar*self.FiArmPilar+10)/100)
    depth=max((max(vuelo/2,0.5)),aux)
    retval+= "$$h = max[v/2; (15\\Phi^2 +10)/100; 0.5] = " + "%3.2f".format(depth) + "\\ m $$\n"
    retval+= "Armadura de la zapata\n"
    Md=(1.5*self.Sadm*(self.Lzapata)**2/8)
    As=(Md/0.8/depth/500*1.15*10)
    retval+= "$$M_d = 1.5 \\sigma_{adm} \\cfrac{a^2}{8}= ","%3.2f".format(Md),"\\ mkN/m $$\n"
    retval+= "$$A_s = \\cfrac{M_d}{0.8hf_{yd}} \\times 10 =" + "%3.2f".format(As),"\\ cm^2/m $$\n"
    retval+= "\\begin{itemize}\n"
    retval+= "\\item La reinforcement se dispondrá en el paramento inferior en ambas direcciones\n"
    retval+= "\\item No es necesario disponer reinforcement en el paramento superior\n"
    retval+= "\\item Se recomienda disponer patilla de, al menos, la mitad del depth de la zapata\n"
    retval+= "\\item Comprobar la reinforcement mínima\n"
    retval+= "\\end{itemize}\n"
    return retval

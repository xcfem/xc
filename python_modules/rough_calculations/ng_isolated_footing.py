...
class IsolatedFooting(object):
  ...
  def getLaTeXResults(self):
    ...
    retval+= "$$A=a^2=\\cfrac{N_k}{\\sigma_{adm}}=" + fmt.Esf.format(area) + "\\ m^2 \\rightarrow a =" + fmt.Length.format(self.Lzapata) + "\\ m$$\n"
    retval+= "Canto de la zapata\n"
    vuelo=((math.sqrt(self.Nk/self.Sadm)-self.Lpilar)/2)
    retval+= "$$ vuelo = v = \\frac{a-l}{2} = " + fmt.Length.format(vuelo) + "\\ m $$\n"
    aux=((15*self.FiArmPilar*self.FiArmPilar+10)/100)
    depth=max((max(vuelo/2,0.5)),aux)
    retval+= "$$h = max[v/2; (15\\Phi^2 +10)/100; 0.5] = " + fmt.Length.format(depth) + "\\ m $$\n"

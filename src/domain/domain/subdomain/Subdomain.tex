...
{\em void commit(void);} \\
invokes the base Domain classes commit() method. It then goes through
...
Sends an ID of size 2 to the channel, sending theAnalysis-$>$getClassTag() and the integer $0$,
...
Receives an ID of size 2 from the channel. If the integer $0$ is sent \p theBroker
...
added using the addNode() command.\\
...
Returns the Matrix obtained from invoking getTangent() on
the DomainDecompositionAnalysis object. \\
...
Returns the Vector obtained by calling getLastSysResponse() on
...
Returns the current value of \p realCost, restting the value of \p realCost to be 0.0. The
value of \p realCost is added to when computeTang() and \p computeREsidual are invoked on the
Subdomain. \\

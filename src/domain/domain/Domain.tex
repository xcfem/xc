...
\indent {\em virtual bool addSP(SFreedom\_Constraint *theSPptr, int pattern);}  \\   
...
\indent {\em virtual  SFreedom\_Constraint *getSFreedom\_ConstraintPtr(int tag);}\\
...
and Nodes, 256 SFreedom\_Constraints and MP\_Constraints, and 32 for the
...
To add the node pointed to by \p theNodePtr to the domain. 
...
invokes setDomain(this)} on the node, {\em domainChange() on
itself, and checks the coordinates of the domain to see if they effect
the box encompassing the Domain. The call returns \p true if the
node was added, otherwise the warning() method of
the global ErrorHandler is invoked and \p false is returned.\\

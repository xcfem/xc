...
// Purpose: This file contains the implementation of class XC::MP_Joint3D.
...
XC::MP_Joint3D::MP_Joint3D(void)
 :MP_Constraint(0, CNSTRNT_TAG_MP_Joint3D ),
...
XC::MP_Joint3D::MP_Joint3D(Domain *theDomain, int tag, int nodeRetain, int nodeConstr,
		int nodeRot, int Rotdof, int nodeDisp, int Dispdof, int LrgDsp )
  :MP_Constraint(tag,nodeRetain,nodeConstr,CNSTRNT_TAG_MP_Joint3D),
  ...
  RotNormVect(3), DspNormVect(3), dbTag3(0), Length0(0.0)
  {
    ...
    if(RnumDOF != 9 || CnumDOF != 6 )
      {
        std::cerr << "MP_Joint3D::MP_Joint3D - mismatch in numDOF\n DOF not supported by this type of constraint";
        return;
      }
    ...
    if( RotDOF<6 || RotDOF>8 || DispDOF<6 || DispDOF>8 || RotDOF==DispDOF )
      {
        std::cerr << "MP_Joint3D::MP_Joint3D - Wrong degrees of freedom" ;
        return;
      }
    ...
    if(dimRet != 3 || dimCon != 3 || dimRot != 3 || dimDsp != 3 )
      {
        std::cerr << "XC::MP_Joint3D::MP_Joint3D - mismatch in dimnesion\n dimension not supported by this type of constraint";
        return;
      }
    ...
    if(Length0 <= 1.0e-12)
      { std::cerr << "XC::MP_Joint3D::MP_Joint3D - The constraint length is zero\n"; }
    ...
    if(RotNormVect.Norm() <= 1.0e-12 || DspNormVect.Norm() <= 1.0e-12 )
      { std::cerr << "XC::MP_Joint3D::MP_Joint3D - the normal vector for the rotation mode or the displacement mode is zero\n"; }
...
bool
XC::MP_Joint3D::isTimeVarying(void) const
{
...
int XC::MP_Joint3D::sendSelf(CommParameters &cp)
  {
    std::cerr << "MP_Joint3D::sendSelf no implementada." << std::endl;
    return 0;
  }


int XC::MP_Joint3D::recvSelf(const CommParameters &cp)
  {
    std::cerr << "MP_Joint3D::recvSelf no implementada." << std::endl;
    return 0;
  }


const XC::Matrix &XC::MP_Joint3D::getConstraint(void)
  {
    if(constraintMatrix.Nula())
      {
        std::cerr << "MP_Joint3D::getConstraint - no XC::Matrix was set\n";
        exit(-1);
      }    
	...
	if ( NewLength < 1e-12 ) std::cerr << "XC::MP_Joint3D::applyConstraint : length of rigid link is too small or zero"; 
...
void XC::MP_Joint3D::Print(std::ostream &s, int flag )
  {
...
void XC::MP_Joint3D::setDomain(Domain *theDomain)
  {
    if(theDomain == nullptr)
      {
        std::cerr << "WARNING MP_Joint3D::SetDomain: Specified domain does not exist";
        std::cerr << "Domain = 0\n";
        return;
      }

...
//! Devuelve el índice de la componente (i,j) del tensor de deformaciones (tensiones)
...
//! @brief Devuelve la componente (i,j) de la tensión media en el elemento.
...
//! @brief Devuelve la deformación media en el elemento.
XC::Vector XC::Brick::getAvgStrain(void) const
  { return physicalProperties.getCommittedAvgStrain(); }

//! @brief Devuelve la componente (i,j) de la deformación media en el elemento.
...
//! como parámetro.
any_const_ptr XC::Brick::GetProp(const std::string &cod) const
  {
    if(verborrea>4)
      std::clog << "Brick::GetProp (" << nombre_clase() 
                << "::GetProp) Buscando propiedad: " << cod << std::endl;

    if(cod == "getAvgStrains")
      {
        static Vector tmp(6);
        tmp= getAvgStrain();
        return get_prop_vector(tmp);
      }
    else if(cod == "getAvgStrain")
      {
        const size_t j= popInt(cod);
        const size_t i= popInt(cod);
        tmp_gp_dbl= getAvgStrain(i,j);
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod == "getAvgStresses")
      {
        static Vector tmp(6);
        tmp= getAvgStress();
        return get_prop_vector(tmp);
      }
    else if(cod == "getAvgStress")
      {
        const size_t j= popInt(cod);
        const size_t i= popInt(cod);
        tmp_gp_dbl= getAvgStress(i,j);
        return any_const_ptr(tmp_gp_dbl);
      }
    else
      return BrickBase::GetProp(cod);
  }

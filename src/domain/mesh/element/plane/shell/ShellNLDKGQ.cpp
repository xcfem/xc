...
XC::Response *XC::ShellNLDKGQ::setResponse(const std::vector<std::string> &argv, Information &eleInformation)
  {
	...
	if(pointNum > 0 && pointNum <= 4)
	  {
	    ...
	    //output.attr("neta",gp.s_coordinate()[pointNum-1]);

	    theResponse= setMaterialResponse(physicalProperties[pointNum-1], argv,2,eleInformation);
        ...
        for(int i=0; i<4; i++)
          {
            ...
            //output.attr("neta",gp.s_coordinate()[i]);
...
int XC::ShellNLDKGQ::getResponse(int responseID, Information &eleInfo)
  {
    int cnt= 0;

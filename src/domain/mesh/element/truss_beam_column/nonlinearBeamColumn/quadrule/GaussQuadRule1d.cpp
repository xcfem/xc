...
bool XC::GaussQuadRule1d::dataSet = false;

double XC::GaussQuadRule1d::ptsArray[maxOrder*(maxOrder+1)/2];
double XC::GaussQuadRule1d::wtsArray[maxOrder*(maxOrder+1)/2];

 XC::Matrix XC::GaussQuadRule1d::pts1 ( ptsArray,     1,  1);
 XC::Matrix XC::GaussQuadRule1d::pts2 (&ptsArray[1],  2,  1);
 XC::Matrix XC::GaussQuadRule1d::pts3 (&ptsArray[3],  3,  1);
 XC::Matrix XC::GaussQuadRule1d::pts4 (&ptsArray[6],  4,  1);
 XC::Matrix XC::GaussQuadRule1d::pts5 (&ptsArray[10], 5,  1);
 XC::Matrix XC::GaussQuadRule1d::pts6 (&ptsArray[15], 6,  1);
 XC::Matrix XC::GaussQuadRule1d::pts7 (&ptsArray[21], 7,  1);
 XC::Matrix XC::GaussQuadRule1d::pts8 (&ptsArray[28], 8,  1);
 XC::Matrix XC::GaussQuadRule1d::pts9 (&ptsArray[36], 9,  1);
 XC::Matrix XC::GaussQuadRule1d::pts10(&ptsArray[45], 10, 1);

 XC::Vector XC::GaussQuadRule1d::wts1 ( wtsArray,     1);
 XC::Vector XC::GaussQuadRule1d::wts2 (&wtsArray[1],  2);
 XC::Vector XC::GaussQuadRule1d::wts3 (&wtsArray[3],  3);
 XC::Vector XC::GaussQuadRule1d::wts4 (&wtsArray[6],  4);
 XC::Vector XC::GaussQuadRule1d::wts5 (&wtsArray[10], 5);
 XC::Vector XC::GaussQuadRule1d::wts6 (&wtsArray[15], 6);
 XC::Vector XC::GaussQuadRule1d::wts7 (&wtsArray[21], 7);
 XC::Vector XC::GaussQuadRule1d::wts8 (&wtsArray[28], 8);
 XC::Vector XC::GaussQuadRule1d::wts9 (&wtsArray[36], 9);
 XC::Vector XC::GaussQuadRule1d::wts10(&wtsArray[45], 10);

XC::GaussQuadRule1d::GaussQuadRule1d()
...
int XC::GaussQuadRule1d::setOrder(int quadOrder)
{
  if (quadOrder < 1 || quadOrder > maxOrder) {
    std::cerr << "XC::GaussQuadRule1d::setOrder() -- Invalid quadrature order " << quadOrder << std::endl;
    exit(-1);
  }
  ...
  default:
    std::cerr << "XC::GaussQuadRule1d::setOrder() -- Invalid quadrature order " << order << std::endl;
    return -1;
    break;
  }    
...
int XC::GaussQuadRule1d::getOrder (void) const
{
  return order;
}

int XC::GaussQuadRule1d::getNumIntegrPoints (void) const
{
  return order;
}

const XC::Matrix & 
XC::GaussQuadRule1d::getIntegrPointCoords (void) const
{
  if (order < 1 || order > maxOrder)
    std::cerr << "XC::GaussQuadRule1d::getIntegrPointWeights() -- order " << order << " is currently invalid\n";

  return *myPts;
}

const XC::Vector & 
XC::GaussQuadRule1d::getIntegrPointWeights (void) const
{
  if (order < 1 || order > maxOrder)
    std::cerr << "XC::GaussQuadRule1d::getIntegrPointWeights() -- order " << order << " is currently invalid\n";
...
const XC::Matrix & 
XC::GaussQuadRule1d::getIntegrPointCoords (int quadOrder)
{
  if (order != quadOrder)
...
const XC::Vector & 
XC::GaussQuadRule1d::getIntegrPointWeights (int quadOrder)
{
  if (order != quadOrder)
    this->setOrder(quadOrder);

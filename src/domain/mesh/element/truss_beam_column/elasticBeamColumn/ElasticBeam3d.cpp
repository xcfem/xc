...
//! @brief Lee un objeto XC::ElasticBeam3d desde archivo
bool XC::ElasticBeam3d::procesa_comando(CmdStatus &status)
  {
    const std::string cmd= deref_cmd(status.Cmd());
    if(verborrea>2)
      std::clog << "(ElasticBeam3d) Procesando comando: " << cmd << std::endl;
    if(cmd == "rho") //Densidad.
      {
      ...
      return ProtoBeam3d::procesa_comando(status);
  }
...
//!
//! Soporta los códigos:
//! -N1: Devuelve la fuerza axil que se ejerce sobre la barra en el extremo dorsal.
//! -N2: Devuelve la fuerza axil que se ejerce sobre la barra en el extremo frontal.
any_const_ptr XC::ElasticBeam3d::GetProp(const std::string &cod) const
  {
    static double L=0.0;
    L= theCoordTransf->getInitialLength();

    if(cod=="length") //Longitud inicial de la barra.
      return any_const_ptr(L);
    else if(cod=="rho") //Densidad de la barra.
      return any_const_ptr(rho);
    else if(cod=="AN1") //fuerza axil que se ejerce sobre la barra en su extremo dorsal.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.AN1()+p0[0];
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AN2") //fuerza axil que se ejerce sobre la barra en su extremo frontal.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.AN2();
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="N1") //axil en el extremo dorsal.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.AN1()-p0[0];
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="N2") //axil en el extremo frontal.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.AN2();
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="N") //axil medio.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= (-q.AN1()-p0[0]+q.AN2())/2.0;
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AMz1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Mz1(); //Momento z que se ejerce sobre la barra en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AMz2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Mz2(); //Momento z que se ejerce sobre la barra en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Mz1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Mz1(); //Momento z en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Mz2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Mz2(); //Momento z en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vy")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vy(L); //Cortante y.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AVy1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vy(L)+p0[1]; //Cortante y que se ejerce sobre la barra en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AVy2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Vy(L)+p0[2]; //Cortante y que se ejerce sobre la barra en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vy1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Vy(L)-p0[1]; //Cortante y en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vy2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vy(L)-p0[2]; //Cortante y en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vz")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vz(L); //Cortante z.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AVz1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vz(L)+p0[3]; //Cortante z que se ejerce sobre la barra en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="AVz2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Vz(L)+p0[4]; //Cortante z que se ejerce sobre la barra en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vz1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= -q.Vz(L)-p0[3]; //Cortante z en su extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="Vz2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.Vz(L)-p0[4]; //Cortante z en su extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="My1")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.My1(); //Momento y en el extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="My2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.My2(); //Momento y en el extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="T") //Torsor en la barra.
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.T();
        return any_const_ptr(tmp_gp_dbl);
      }
      ...
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.T1(); //+p0[0]; //Torsor en el extremo dorsal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="T2")
      {                 //¡Ojo! antes hay que llamara a "calc_resisting_force".
        tmp_gp_dbl= q.T2(); //Torsor en el extremo frontal.
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="getVDirEjeFuerteLocales")
      return get_prop_vector(getVDirEjeFuerteLocales());
    else if(cod=="getVDirEjeDebilLocales")
      return get_prop_vector(getVDirEjeDebilLocales());
    else if(cod=="getAnguloEjeFuerte")
      {
        tmp_gp_dbl= getAnguloEjeFuerte();
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="getAnguloEjeDebil")
      {
        tmp_gp_dbl= getAnguloEjeDebil();
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="getVDirEjeFuerteGlobales")
      return get_prop_vector(getVDirEjeFuerteGlobales());
    else if(cod=="getVDirEjeDebilGlobales")
      return get_prop_vector(getVDirEjeDebilGlobales());
    else
      return ProtoBeam3d::GetProp(cod);
  }

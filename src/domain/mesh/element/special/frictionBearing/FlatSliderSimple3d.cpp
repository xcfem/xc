...
void XC::FlatSliderSimple3d::setDomain(Domain *theDomain)
  {
    ...
    if(dofNd1 != 6)
      {
	std::cerr << "XC::FlatSliderSimple3d::setDomain() - node 1: "
		  << " has incorrect number of DOF (not 6)\n";
	return;
      }
    if(dofNd2 != 6)
      {
	std::cerr << "FlatSliderSimple3d::setDomain() - node 2: "
			<< " has incorrect number of DOF (not 6)\n";
	return;
      }
...
int XC::FlatSliderSimple3d::update()
{
    ...
    if (iter >= maxIter)   {
        std::cerr << "WARNING: FlatSliderSimple3d::update() - did not find the shear force after "
...
{  
	std::cerr <<"FlatSliderSimple3d::addLoad() - "
...
int XC::FlatSliderSimple3d::addInertiaLoadToUnbalance(const Vector &accel)
{
	...
	if (6 != Raccel1.Size() || 6 != Raccel2.Size())  {
		std::cerr << "FlatSliderSimple3d::addInertiaLoadToUnbalance() - "
...
const XC::Vector& XC::FlatSliderSimple3d::getResistingForce()
{
    // zero the residual
    theVector.Zero();
...
const XC::Vector &XC::FlatSliderSimple3d::getResistingForceIncInertia()
  {	
...
void XC::FlatSliderSimple3d::Print(std::ostream &s, int flag)
{
    if (flag == 0)  {
        // print everything
...
{ 
    ...
    if (L > DBL_EPSILON)  {
		if (x.Size() == 0)  {
            ...
            std::cerr << "WARNING FlatSliderSimple3d::setUp() - " 
    ...
    if (x.Size() != 3 || y.Size() != 3)  {
        std::cerr << "FlatSliderSimple3d::setUp() - "
            << "element: " << this->getTag() << std::endl

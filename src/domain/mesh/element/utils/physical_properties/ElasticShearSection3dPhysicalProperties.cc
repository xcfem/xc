...
//----------------------------------------------------------------------------
//ElasticShearSection3dPhysicalProperties.cpp

#include "ElasticShearSection3dPhysicalProperties.h"
#include <domain/domain/Domain.h>
#include "domain/mesh/element/utils/Information.h"
#include "utility/recorder/response/ElementResponse.h"

//! @brief Assigns the given section properties to the integration point i
void XC::ElasticShearSection3dPhysicalProperties::set(const size_t &i, const CrossSectionProperties3d &sp)
   { theMaterial[i]->setCrossSectionProperties(sp); }

//! @brief Assigns the given section properties to all the integration points.
void XC::ElasticShearSection3dPhysicalProperties::set(const CrossSectionProperties3d &sp)
   {
...
XC::Vector XC::ElasticShearSection3dPhysicalProperties::getGeneralizedStressAtGaussPointsByName(const std::string &cod) const
  {
    const size_t nMat= theMaterial.size();
    Vector retval(nMat);
    for(size_t i= 0; i<nMat; i++)
      {
	const ElasticShearSection3d *section= dynamic_cast<const ElasticShearSection3d *>(theMaterial[i]);
...
//!
//! @param cod component code.
XC::Vector XC::ElasticShearSection3dPhysicalProperties::getGeneralizedStrainAtGaussPointsByName(const std::string &cod) const
  {
    const size_t nMat= theMaterial.size();
    Vector retval(nMat);
    for(size_t i= 0; i<nMat; i++)

...
//----------------------------------------------------------------------------
//ElasticShearSection2dPhysicalProperties.cpp

#include "ElasticShearSection2dPhysicalProperties.h"
#include <domain/domain/Domain.h>
#include "domain/mesh/element/utils/Information.h"
#include "utility/recorder/response/ElementResponse.h"

//! @brief Assigns section properties for integration point i
void XC::ElasticShearSection2dPhysicalProperties::set(const size_t &i, const CrossSectionProperties2d &sp)
   { theMaterial[i]->setCrossSectionProperties(sp); }

//! @brief Assigns the given section properties to all the integration points.
void XC::ElasticShearSection2dPhysicalProperties::set(const CrossSectionProperties2d &sp)
   {
...
XC::Vector XC::ElasticShearSection2dPhysicalProperties::getGeneralizedStressAtGaussPointsByName(const std::string &cod) const
  {
    const size_t nMat= theMaterial.size();
    Vector retval(nMat);
    for(size_t i= 0; i<nMat; i++)
      {
	const ElasticShearSection2d *section= dynamic_cast<const ElasticShearSection2d *>(theMaterial[i]);
...
//!
//! @param cod component code.
XC::Vector XC::ElasticShearSection2dPhysicalProperties::getGeneralizedStrainAtGaussPointsByName(const std::string &cod) const
  {
    const size_t nMat= theMaterial.size();
    Vector retval(nMat);
    for(size_t i= 0; i<nMat; i++)

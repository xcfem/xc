...
//! @brief Lee un objeto TritrizPtrBase desde archivo
//! Soporta los comandos:
template <class MatrizPtr>
bool TritrizPtrBase<MatrizPtr>::procesa_comando(CmdStatus &status)
  {
    const std::string cmd= deref_cmd(status.Cmd());
    if(verborrea>2)
      std::clog << "(TritrizPtrBase) Procesando comando: " << cmd << std::endl;

    if(cmd == "capa")
      {
        size_t icapa= 1;
        const CmdParser &parser= status.Parser();
        if(parser.TieneIndices()) //No se indica capa.
          {
            interpreta(parser.GetIndices());
            if(InterpreteRPN::HayArgumentos(1,cmd))
              icapa= convert_to_size_t(InterpreteRPN::Pila().Pop());
          }
        else
	  std::cerr << "(TritrizPtrBase) Procesando comando: '" << cmd
                            << "'; falta el índice de la capa.\n";
        (*this)[icapa-1].LeeCmd(status);
        return true;
      }
    else if(cmd == "obj")
      {
            ...
            if(InterpreteRPN::HayArgumentos(3,cmd))
              {
                ...
                value_type ptr= (*this)(i,j,k);
                if(ptr)
                  ptr->LeeCmd(status);
                else
	          std::clog << "(TritrizPtrBase) Procesando comando: '" << cmd
                            << "'; no se encontró el punto de índices: (" << i << ',' << j << ',' << k << ").\n";
              }
          }
        else
	  std::cerr << "(TritrizPtrBase) Procesando comando: '" << cmd
	...
	for(;i!= this->end();i++)
          (*i).EjecutaBloqueForEach(status,bloque);
        return true;
      }
    else if(cmd == "dim")
      {
	std::vector<size_t> dim(crea_vector_size_t(status.GetString()));
	    ...
	    std::clog << "(MatrizPtrBase) Procesando comando: '" << cmd
                      << "'; se esperaban tres valores."
                      << posLectura << std::endl;
          }
        this->resize(dim[0],dim[1],dim[2],NULL);
        return true;
      }
    else if(cmd == "objs_capa")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        ...
        /*     if(InterpreteRPN::HayArgumentos(1,cmd)) */
        /*       { */
        /*         const size_t capa= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsCapa(capa,status,bloque); */
        /*       } */
        /*   } */
        /* else */
    ...
    else if(cmd == "objs_interiores_capa")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        /* const std::string &bloque= status.GetBloque(); */
        /* const CmdParser &parser= status.Parser(); */
        ...
        /*       { */
        /*         const size_t capa= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsInterioresCapa(capa,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
	...
	std::cerr << "deprecated. Use Python." << std::endl;
        ...
        /*         EjecutaBloqueObjsFila(capa,f,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
        /*                     << "'; falta(n) el(los) índice(s) de capa y fila.\n"; */
        return true;
      }
    else if(cmd == "objs_interiores_fila")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        /* const std::string &bloque= status.GetBloque(); */
        /* const CmdParser &parser= status.Parser(); */
        ...
        /*         const size_t capa= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         const size_t f= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsInterioresFila(capa,f,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
	...
	std::cerr << "deprecated. Use Python." << std::endl;
        ...
        /*       { */
        /*         const size_t f= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsFilas(f,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
        /*                     << "'; falta el índice de la fila.\n"; */
        return true;
      }
    else if(cmd == "objs_interiores_filas")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        /* const std::string &bloque= status.GetBloque(); */
        /* const CmdParser &parser= status.Parser(); */
        ...
        /*       { */
        /*         const size_t f= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsInterioresFilas(f,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
	...
	std::cerr << "deprecated. Use Python." << std::endl;
        ...
        /*         EjecutaBloqueObjsColumna(capa,c,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
        /*                     << "'; falta(n) el(los) índice(s) de capa y columna.\n"; */
        return true;
      }
    else if(cmd == "objs_interiores_columna")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        /* const std::string &bloque= status.GetBloque(); */
        /* const CmdParser &parser= status.Parser(); */
        ...
        /*         const size_t capa= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         const size_t c= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsInterioresColumna(capa,c,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
	...
	std::cerr << "deprecated. Use Python." << std::endl;
        ...
        /*       { */
        /*         const size_t c= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsColumnas(c,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
    ...
    else if(cmd == "objs_interiores_columnas")
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        /* const std::string &bloque= status.GetBloque(); */
        /* const CmdParser &parser= status.Parser(); */
        ...
        /*       { */
        /*         const size_t c= convert_to_size_t(InterpreteRPN::Pila().Pop()); */
        /*         EjecutaBloqueObjsInterioresColumnas(c,status,bloque); */
        /*       } */
        /*   } */
        /* else */
	/*   std::cerr << "(MatrizPtrBase) Procesando comando: '" << cmd */
      ...
      {
	std::cerr << "deprecated. Use Python." << std::endl;
        //ForEachInteriorObj(status,status.GetBloque());
        return true;
      }
    else

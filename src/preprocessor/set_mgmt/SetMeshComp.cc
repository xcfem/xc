...
bool XC::SetMeshComp::procesa_comando(CmdStatus &status)
  {
	...
	std::vector<int> tags_nodos= crea_vector_int(status.GetString());
        ...
        if(preprocessor)
          for(size_t i= 0;i<sz;i++)
            {
              nodos.push_back(preprocessor->GetDominio()->getNode(tags_nodos[i]));
            }
        return true;
      }
          ...
          for(size_t i= 0;i<sz;i++)
            {
              ...
              nodos.push_back(preprocessor->GetDominio()->getNode(tag));
            }
        return true;
      }
            ...
            if(fnc_indices.size()>0)
              {
                ...
                set_base= dynamic_cast<SetMeshComp *>(preprocessor->get_sets().busca_set(nmbSetBase));
                if(!set_base)
                  {
	...
	std::vector<int> tags_elementos= crea_vector_int(status.GetString());
        ...
        if(preprocessor)
          for(size_t i= 0;i<sz;i++)
            {
              elementos.push_back(preprocessor->GetDominio()->getElement(tags_elementos[i]));
            }
        return true;
      }
	...
	const Lista tmp= interpretaLista(status.GetString());
        ...
        if(preprocessor)
          for(size_t i= 0;i<sz;i++)
            {
              tag= convert_to_int(tmp[i]);
              elementos.push_back(preprocessor->GetDominio()->getElement(tag));
            }
        return true;
      }

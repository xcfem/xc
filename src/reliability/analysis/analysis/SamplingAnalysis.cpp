...
int XC::SamplingAnalysis::analyze(void)
  {
    ...
    LimitStateFunction *theLimitStateFunction = nullptr;
    NormalRV *aStdNormRV= nullptr;
    ...
    double factor1 = 1.0 / ( pow((2.0*M_PI),((double)numRV/2.0)) );
    double factor2 = 1.0 / ( pow((2.0*M_PI),((double)numRV/2.0)) * sqrt(det_covariance) );
	...
	for (int lsf=0; lsf<numLsf; lsf++ ) {
	  ...
	  if (analysisTypeTag == 1) {
	    ...
	    resultsOutputFile << gFunctionValue << "  ";
	  ...
	  for(int lsf=0; lsf<numLsf; lsf++ )
	    {
	      ...
	      outputFile << q_bar(lsf) << " " << cov_of_q_bar(lsf) << " " << std::endl;
	    }
	  outputFile.close();
	}

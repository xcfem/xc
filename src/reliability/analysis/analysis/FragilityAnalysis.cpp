...
#include <reliability/analysis/analysis/FragilityAnalysis.h>
...
//! @brief Constructor.
XC::FragilityAnalysis::FragilityAnalysis(ReliabilityDomain *passedReliabilityDomain,
...
int XC::FragilityAnalysis::analyze(void)
  {
 ...
 for ( ; lsf<=numLsf; lsf++ ) {
		...
		if (theLimitStateFunction == 0) {
			std::cerr << "XC::FragilityAnalysis::analyze() - could not find" << std::endl
		...
		for (int counter=0; counter<(numIntervals+1); counter++) {
			...
			if (numPos != 0) {
				...
				if (theParameterPositioner == 0) {
					std::cerr << "XC::FragilityAnalysis::analyze() -- The parameter number in the " << std::endl
				...
				while ( tokenPtr != nullptr ) {
					if ( strcmp(tokenPtr, "par") == 0) {
						...
						if (par==parameterNumber) {
							...
							std::cerr << "XC::FragilityAnalysis::analyze() -- The parameter number " << std::endl

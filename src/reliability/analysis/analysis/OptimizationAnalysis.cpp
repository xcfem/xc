...
double XC::OptimizationAnalysis::g(Vector x, Vector u, int k) 
 {

	// Probability transformations
	Vector v(8);
	v(1-1) = u(1-1)*0.15*413.4*1e6 + 413.4*1e6;       
	v(2-1) = u(2-1)*0.15*27.56*1e6 + 27.56*1e6;   
	v(3-1) = u(3-1)*0.2*13.57*1e3 + 13.57*1e3;      
	v(4-1) = u(4-1)*0.243*929*1e3 + 929*1e3;      
	v(5-1) = u(5-1)*0.243*138.31*1e3 + 138.31*1e3; 
	v(6-1) = u(6-1)*0.243*183.39*1e3 + 183.39*1e3; 
	v(7-1) = u(7-1)*0.243*228.51*1e3 + 228.51*1e3; 
	v(8-1) = u(8-1)*0.1*22.74*1e3 + 22.74*1e3;     

	double Y3 = 18.30;
	double alpha = 0.1;

	double X15 = (2*x(4-1)/0.0254*(x(3-1)/0.0254 + x(5-1)/0.0254 - alpha/0.0254)*sqrt(v(2-1)/(6.89*1e3)))*4.45; 
	...
	case 5:
		...
		double X13 = (0.85*0.85*v(2-1)/v(1-1))*(87/(87 + v(1-1)/(6.89*1e6)));
		gval = -X12 + 0.75*X13;
		break;
	case 6:
		double X12 = x(1-1)/(x(4-1)*(x(3-1) + x(5-1) - alpha));
		double X14 = 0.85*v(2-1)*(x(2-1) - x(4-1))*x(3-1)/v(1-1); 
		double X13 = ((0.85*0.85*v(2-1)/v(1-1))*87/(87 + v(1-1)/(6.89*1e6)) + X14/(x(4-1)*(x(3-1) + x(5-1) - alpha)))*x(4-1)/x(2-1);
		gval = -X12 + 0.75*X13;
		break;
	case 7:
		double X21 = 200/(1000*v(1-1)/(6.89*1e6));  
		double X12 = x(1-1)/(x(2-1)*(x(3-1) + x(5-1) - alpha));
		gval = -X21 + X12;
		break;
	case 8:
		...
		break;
	case 9:
		gval = -x(6-1)*(v(1-1)/1e3/6.89)/x(9-1)/2/x(4-1)*(1/sqrt(v(2-1)/(6.89*1e3))) + 4; 
		break;
	case 10:

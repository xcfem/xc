...
XC::NatafProbabilityTransformation::NatafProbabilityTransformation(ReliabilityDomain *passedReliabilityDomain,
                                                                                         int passedPrintFlag)
:ProbabilityTransformation()
{
        ...
        if (result < 0) {
                std::cerr << "XC::NatafProbabilityTransformation::NatafProbabilityTransformation() - could not" << std::endl
                        << " compute the Cholesky decomposition and its inverse " << std::endl
                        << " for the correlation matrix." << std::endl;
        }
        (*lowerCholesky)= theMatrixOperations->getLowerCholesky();
        (*inverseLowerCholesky)= theMatrixOperations->getInverseLowerCholesky();
}

XC::NatafProbabilityTransformation::~NatafProbabilityTransformation()
{
...
int 
XC::NatafProbabilityTransformation::set_x(Vector passedx)
{
...
int 
XC::NatafProbabilityTransformation::set_u(Vector passedu)
{
...
int 
XC::NatafProbabilityTransformation::transform_x_to_u()
{
        Vector z= x_to_z(*x);
 ...
 XC::Vector 
XC::NatafProbabilityTransformation::meanSensitivityOf_x_to_u(Vector &x, int rvNumber)
{
        ...
        if((theRV->getType()=="NORMAL"))
        ...
        else if((theRV->getType()=="LOGNORMAL"))
          {
            ...
            const double mean= fabs(theRV->getMean()); // more here for negative lognormal?
            const double stdv= theRV->getStdv();

            const double a= mean*mean+stdv*stdv;
            DzDmean(rvNumber-1)= 0.5*(-2.0*mean*mean*log(a)+4.0*mean*mean*log(mean)-3.0*stdv*stdv*log(a)+4.0*stdv*stdv*log(mean)+2.0*stdv*stdv*log(fabs(x(rvNumber-1))))/(pow(log(a)-2.0*log(mean),1.5)*mean*a);
...
//          DzDmean(rvNumber-1)= f*(-d-e+e*z/zeta);
          ...
          {
            std::cerr << "WARNING: Cannot compute reliability sensitivity results for " << std::endl
                      << " type of random variable number " << rvNumber << std::endl;
 ...
 XC::Vector 
XC::NatafProbabilityTransformation::stdvSensitivityOf_x_to_u(Vector &x, int rvNumber)
{
        // Returns the sensitivity of 'u' with respect to [mean, stdv]
        // for the given random variable number (rvNumber)
...
int 
XC::NatafProbabilityTransformation::transform_u_to_x()
{
...
int 
XC::NatafProbabilityTransformation::transform_u_to_x_andComputeJacobian()
{
        Vector z= (*lowerCholesky) * (*u);
        (*x)= z_to_x(z);
        ...
        if (result < 0) {
                std::cerr << "XC::NatafProbabilityTransformation::transform_u_to_x() - could not set " << std::endl
                        << " the matrix in the object to perform matrix operations." << std::endl;
                return -1;
        }
        
        result= theMatrixOperations->computeInverse();
        if (result < 0) {
                std::cerr << "XC::NatafProbabilityTransformation::transform_u_to_x() - could not " << std::endl
                        << " invert Jacobian_u_x." << std::endl;
                return -1;
        }
 ...
 XC::Vector 
XC::NatafProbabilityTransformation::get_x()
{
        return (*x);
}
 ...
 XC::Vector 
XC::NatafProbabilityTransformation::get_u()
{
        return (*u);
}
 ...
 XC::Matrix 
XC::NatafProbabilityTransformation::getJacobian_x_u()
{
        return (*jacobian_x_u);
}
 ...
 XC::Matrix 
XC::NatafProbabilityTransformation::getJacobian_u_x()
{
        return (*jacobian_u_x);
}
 ...
 XC::Matrix
XC::NatafProbabilityTransformation::getJacobian_z_x(Vector x, Vector z)
{        
        RandomVariable *theRV;
        NormalRV aStandardNormalRV(1, 0.0, 1.0, 0.0);
                ...
                if ((theRV->getType()=="NORMAL")) {
                        ...
                        if (x(i) == 0.0) {
                                std::cerr << "XC::NatafProbabilityTransformation::getJacobian_z_x() -- Error: value " << std::endl
                                    << "of lognormal random variable is zero in original space. " << std::endl;
                        }
                        ...
                        if (pdf == 0.0) {
                                std::cerr << "ERROR: XC::NatafProbabilityTransformation::getJacobian_z_x() -- " << std::endl
                                        << " the PDF value is zero, probably due to too large step. " << std::endl;
                        }
 ...
 XC::Vector
XC::NatafProbabilityTransformation::z_to_x(Vector z)
{
        RandomVariable *theRV;
        NormalRV aStandardNormalRV(1, 0.0, 1.0, 0.0);
        Vector x(nrv);
 ...
 XC::Vector
XC::NatafProbabilityTransformation::x_to_z(Vector x)
{
        RandomVariable *theRV;
        NormalRV aStandardNormalRV(1, 0.0, 1.0, 0.0);
        Vector z(nrv);
...
void XC::NatafProbabilityTransformation::setCorrelationMatrix(int pertMeanOfThisRV, int pertStdvOfThisRV, double h)
{
        ...
        for ( int j=1 ; j<=numberOfCorrelationCoefficients ; j++ )
        {
                ...
                if (rv1Ptr->getMean() == 0.0  ||  rv2Ptr->getMean() == 0.0 ) {
                        std::cerr << "WARNING: The Nataf-transformation is currently implemented " << std::endl
                                << " with the assumption of non-zero mean. " << std::endl;
                }
                ...
                else if( ((typeRv2=="TYPE3SMALLESTVALUE") || (typeRv2=="WEIBULL"))  &&  ( (typeRv2=="TYPE3SMALLESTVALUE")  || (typeRv2=="WEIBULL") ) )
		  {
                ...
                /////////////////////////////////////////////////////////////////////////////////
                        ...
                        std::cerr << "Did not find the given combination of distributions in CorrelationModifier" << std::endl;
                }
...
double
XC::NatafProbabilityTransformation::integrand(int rv_i,
                                                                                  double z_i, 
                                                                                  double mean_i,
                                                                                  double stdv_i, 
                                                                                  int rv_j,
...
double
XC::NatafProbabilityTransformation::doubleIntegral(int rv_i,
                                                                                  double mean_i,
                                                                                  double stdv_i, 
                                                                                  int rv_j,
                                                                                  double mean_j, 
...
double
XC::NatafProbabilityTransformation::residualFunction(double rho_original, 
                                                                                                 double rho,
                                                                                                 double rv_i, 
                                                                                                 double mean_i, 
                                                                                                 double stdv_i, 
...
double
XC::NatafProbabilityTransformation::solveForCorrelation(int rv_i, int rv_j, double rho_original)
{
        RandomVariable *theRv_i= theReliabilityDomain->getRandomVariablePtr(rv_i);
        RandomVariable *theRv_j= theReliabilityDomain->getRandomVariablePtr(rv_j);
        ...
        for (int i=1;  i<=100;  i++ )  {
                ...
                if ( fabs(df) < 1.0e-15) {
                        std::cerr << "WARNING: XC::NatafProbabilityTransformation::solveForCorrelation() -- " << std::endl
                                << " zero derivative in Newton algorithm. " << std::endl;
                }
                else {
                        ...
                        if (fabs(1.0-fabs(rho_old/rho_new)) < tol) {
                                ...
                                if (i==100) {
                                        std::cerr << "WARNING: XC::NatafProbabilityTransformation::solveForCorrelation() -- " << std::endl
                                                << " Newton scheme did not converge. " << std::endl;
                                        result= 0.0;
                                        return result;
                                }
                                else {

...
int XC::SearchWithStepSizeAndStepDirection::findDesignPoint(ReliabilityDomain *passedReliabilityDomain)
  {
	...
	while(i <= maxNumberOfIterations)
	  {
	    ...
	    if(result < 0)
	      {
	        std::cerr << "XC::SearchWithStepSizeAndStepDirection::doTheActualSearch() - "
			  << std::endl
			  << " could not transform from u to x and compute Jacobian."
			  << std::endl;
		return -1;
	      }
		...
		if(result < 0)
		  {
		    std::cerr << "XC::SearchWithStepSizeAndStepDirection::doTheActualSearch() - "
			      << std::endl
			      << " could not run analysis to evaluate limit-state function. "
			      << std::endl;
		    return -1;
		  }
    ...
    std::cerr << "Maximum number of iterations was reached before convergence."
	      << std::endl;
...
const XC::Vector &XC::SearchWithStepSizeAndStepDirection::get_x(void) const
  { return x; }

const XC::Vector &XC::SearchWithStepSizeAndStepDirection::get_u(void) const
  { return u; }

const XC::Vector &XC::SearchWithStepSizeAndStepDirection::get_alpha(void) const
  { return alpha; }

XC::Vector XC::SearchWithStepSizeAndStepDirection::get_gamma(void) const
  { return gamma*(1.0/gamma.Norm()); }

int XC::SearchWithStepSizeAndStepDirection::getNumberOfSteps(void) const
  { return (i-1); }

const XC::Vector &XC::SearchWithStepSizeAndStepDirection::getSecondLast_u(void) const
  { return uSecondLast; }

const XC::Vector &XC::SearchWithStepSizeAndStepDirection::getSecondLast_alpha(void) const
  { return alphaSecondLast; }

const XC::Vector &XC::SearchWithStepSizeAndStepDirection::getLastSearchDirection(void) const
  { return searchDirection; }

double XC::SearchWithStepSizeAndStepDirection::getFirstGFunValue(void) const
  { return Gfirst; }

double XC::SearchWithStepSizeAndStepDirection::getLastGFunValue(void) const
  { return Glast; }


const XC::Vector &XC::SearchWithStepSizeAndStepDirection::getGradientInStandardNormalSpace(void) const
  { return gradientInStandardNormalSpace; }



int XC::SearchWithStepSizeAndStepDirection::getNumberOfEvaluations(void) const

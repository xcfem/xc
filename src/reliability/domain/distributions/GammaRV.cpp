...
XC::GammaRV::GammaRV(int passedTag, 
                 double passedMean,
                 double passedStdv,
...
double
XC::GammaRV::getPDFvalue(double rvValue)
{
        double result;
        if ( 0.0 < rvValue ) {
...
double
XC::GammaRV::getCDFvalue(double rvValue)
{
        double result;
        if ( 0.0 < rvValue ) {
...
double
XC::GammaRV::getInverseCDFvalue(double probValue)
{
        double result = 0.0;
...
double XC::GammaRV::getParameter1()  {return k;}
double XC::GammaRV::getParameter2()  {return lambda;}
double XC::GammaRV::getParameter3()  {std::cerr<<"No such parameter in r.v. #"<<tag<<std::endl; return 0.0;}
double XC::GammaRV::getParameter4()  {std::cerr<<"No such parameter in r.v. #"<<tag<<std::endl; return 0.0;}





double XC::GammaRV::gammaFunction(double x)
  {
    double res= 0.0;

    if(x==0 || ( x < 0.0 && floor(x)==x ) )
      { std::cerr << "Invalid input to the gamma function" << std::endl; }
    else
      {
        Vector p(9);
        Vector q(9);
...
double 
XC::GammaRV::incompleteGammaFunction(double x, double a)
{

...
// $Source: /usr/local/cvs/OpenSees/SRC/analysis/model/fe_ele/lagrange/LagrangeMP_FE.cpp,v $
...
// of the XC::LagrangeMP_FE class interface.
...
XC::LagrangeMP_FE::LagrangeMP_FE(int tag, Domain &theDomain, MP_Constraint &TheMP, DOF_Group &theGroup, double Alpha)
  :MP_FE(tag, 3,(TheMP.getConstrainedDOFs().Size()+TheMP.getRetainedDOFs().Size()+TheMP.getRetainedDOFs().Size()),TheMP,Alpha), Lagrange_FE(theGroup)
  {
    ...
    if(theRetainedNode == 0)
      {
	std::cerr << "WARNING XC::LagrangeMP_FE::LagrangeMP_FE()";
...
int XC::LagrangeMP_FE::setID(void)
  {
    int offset = 0;
...
const XC::Matrix &XC::LagrangeMP_FE::getTangent(Integrator *theNewIntegrator)
  {
...
const XC::Vector &XC::LagrangeMP_FE::getResidual(Integrator *theNewIntegrator)
  { return resid; }



const XC::Vector &XC::LagrangeMP_FE::getTangForce(const Vector &disp, double fact)
  {
    std::cerr << "WARNING LagrangeMP_FE::getTangForce() - not yet implemented\n";
    resid.Zero(); //Añadida LCPT.
    return  resid;
  }

const XC::Vector &XC::LagrangeMP_FE::getK_Force(const Vector &disp, double fact)
  {
    std::cerr << "WARNING LagrangeMP_FE::getK_Force() - not yet implemented\n";
    resid.Zero(); //Añadida LCPT.
    return resid;
  }

const XC::Vector &XC::LagrangeMP_FE::getC_Force(const Vector &disp, double fact)
  {
    std::cerr << "WARNING LagrangeMP_FE::getC_Force() - not yet implemented\n";
    resid.Zero(); //Añadida LCPT.
    return resid;
  }

const XC::Vector &XC::LagrangeMP_FE::getM_Force(const XC::Vector &disp, double fact)
  {
    std::cerr << "WARNING LagrangeMP_FE::getM_Force() - not yet implemented\n";
    resid.Zero(); //Añadida LCPT.
    return resid;
  }

void XC::LagrangeMP_FE::determineTangent(void)
  {
    const Matrix &constraint = theMP->getConstraint();
    const int noRows= constraint.noRows();
    const int noCols= constraint.noCols();

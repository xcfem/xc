...
//! @brief Constructor.
XC::MRMP_FE::MRMP_FE(int tag, int numDOF_Group, int ndof, MRMP_Constraint &TheMP,const double &Alpha)
...
int XC::MRMP_FE::determineConstrainedDOFsIDs(const int &offset)
  { return MPBase_FE::determineConstrainedDOFsIDs(*theMRMP,offset); }

//! @brief determine determine the IDs for the retained dof's
int XC::MRMP_FE::determineRetainedDOFsIDs(const int &offset)
  {
    int retval= offset;
    ...
    for(size_t i= 0;i<sz;i++)
      {
        ...
        if(theRetainedNodesDOFs == 0)
          {
	    std::cerr << "WARNING XC::MRMP_FE::setID(void)";
	    ...
	    if(retained < 0 || retained >= theRetainedNode->getNumberDOF())
              {
	        std::cerr << "WARNING XC::MRMP_FE::setID(void) - unknown DOF ";
...
int XC::MRMP_FE::determineRetainedNodesDofGrpPtr(Domain &theDomain, const int &offset)
  {
    const ID &retainedNodeTags= theMRMP->getRetainedNodeTags();
    const size_t sz= retainedNodeTags.Size();
    ...
    for(size_t i= 0;i<sz;i++)
      {
        ...
        if(theRetainedNode == 0)
          {
	    std::cerr << "FATAL MRMP_FE::determineRetainedNodesDofGrpPtr() - Retained";
        ...
        if(dofGrpPtr!= 0)
          ...
          std::cerr << "WARNING MRMP_FE::MRMP_FE() - node no Group yet?\n"; 
      }
    return offset+sz;
  }

...
constructor. The value of $\alpha$ is set to \p alpha and 
$\Delta s$ to \p dS. \\
...
$\P$, invoke formTangent() on itself and solve the LinearSOE to
get $\Delta \Uh_n^{(1)}$. \\
...
vector of size $2$ and invokes sendVector() on \p theChannel.
Returns $0$ if successful, a warning message is printed and
a $-1$ is returned if \p theChannel fails to send the Vector. \\
...
\lambda$ is set to $0$, and a $-1$ is returned if \p theChannel

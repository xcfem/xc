...
$t + \Delta t$ (between newStep()} and {\em commit() the $t +
...
associated with the object and $-2$ if commitDomain() failed. \\
...
$-1$ is returned if \p theChannel fails to send the Vector. \\ 
...
rayleigh damping flag to \p false, and a $-1$ is returned if {\em

...
$\Delta \U$, if \p dispFlag is \p true, or  
acceleration, $\Delta \ddot \U$, if \p dispFlag is \p false. In
addition, a flag is set indicating that Rayleigh damping will not be used. \\


\indent {\em Newmark(double gamma, double beta, bool dispFlag = true);}\\ 
Sets $\gamma$ to \p gamma and $\beta$ to \p beta. Sets a flag
...
Sets $\gamma$ to \p gamma, $\beta$ to \p beta, $\alpha_M$ to
...
the flag is \p false {\em c1} is set to $\beta \Delta t^2$, {\em c2} to $
...
$\beta$ to 0.25 and the Rayleigh damping flag set to \p false, and
a $-1$ is returned, if \p theChannel fails to receive the Vector.\\ 

 ...
 ************************************************************************
    ...
    output parameters -
        (nperm,ninvp) - new_ permutation and inverse permutation vectors
 ...
 ************************************************************************
    ...
    purpose - to form an equivalent post ordering of the elimination
	      ...
	      new_ blocks in the generalized profile matrix.
    ...
    updated parameters -
        ninvp -  new_ invp vector
        nperm -  new_ perm vector
        parent - new_ parent vector 
...
/* ----------------------------------------------------------
   ...
   {  /* add count to list of nodes beginning a new_ block */
      xcount++ ;
      list[xcount] = count;
   }
/* --------------------------------------------------------
   ...
   nperm[count] = t ;
   ninvp[t] = count ;
/* save new_ number in t to update the parent node vector after sibligns are
   renumbered and the value of the new_ parent number is determined */ 
   t = count ;
...
/* update subtrees of siblings */
   ...
   {  postordr(sibling[i],parent,fchild, sibling, oinvp, operm, 
	 ninvp, nperm, list, rowblks ) ;
      /* add parent to list of nodes beginning a new_ block */

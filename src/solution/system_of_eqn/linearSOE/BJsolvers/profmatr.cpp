...
//XC::tempoutprofilematrix::profilematrix(FEModelData & FEMD, Brick3D * b3d, Node *  node)
//tempout//XC::skymatrix::skymatrix(FEModelData & FEMD, Finite_Element & FE)
//tempout  {
//tempout// create the structure:
//tempout     pc_profilematrix_rep=new_ profilematrix_rep; // this 'new' is overloaded
//tempout
//tempout
...
//tempout    pc_profilematrix_rep->columnheight=new_ int [Number_of_DOFs+1];
...
//tempout    pc_profilematrix_rep->jp=new_ int [Number_of_DOFs];
...
//tempout//  void XC::profilematrix::profile_init( FEModelData      & FEMD)
//tempout//    {
...
//tempout    pc_profilematrix_rep->al=new_ double [(size_t) length_of_AL];
//tempout      if (!pc_profilematrix_rep->al)
//tempout        { printf("\a\nInsufficient memory for AL\n"); exit(1);}
//tempout    pc_profilematrix_rep->ad=new_ double [(size_t) Numb_of_DOF];
//tempout      if (!pc_profilematrix_rep->ad)
//tempout        { printf("\a\nInsufficient memory for AD\n"); exit(1);}
//tempout    pc_profilematrix_rep->au=new_ double [(size_t) length_of_AL];
...
//tempout//--
//tempout//--    pc_profilematrix_rep->data=new_ double [Total_K_length+1];
//tempout//--
...
XC::profilematrix::profilematrix(int matrix_order,
                             int *jp_init,
                             char flag_init,
                             double *au_init_val,
                             double *al_init_val,
...
//##############################################################################
XC::profilematrix::profilematrix(const profilematrix & x)   // copy initializer
 {
  x.pc_profilematrix_rep->n++; // we're adding another reference.
  pc_profilematrix_rep= x.pc_profilematrix_rep;  // point to the new_ profilematrix_rep.
 }



//##########################################################################
XC::profilematrix::~profilematrix(void)
  {
...
//##########################################################################
int XC::profilematrix::dimension_of_profile_M(void) const // dimension of profile matrix
  {
    return pc_profilematrix_rep->neq;
  }

//##########################################################################
int * XC::profilematrix::get_jp(void) const  // get pointer to array of
...
double XC::profilematrix::get_max(void) // 
 {                                                 
...
void XC::profilematrix::set_penalty_element(int *eqn_no_shake, int no_of_shake, double max_element) 
 {                                                  
...
//##########################################################################
//double  XC::profilematrix::get_max(void)        // get pointer to array of
//  {                                      // Locations of Diagonals
//    return pc_profilematrix_rep->max_element;
//  }
//##########################################################################
//double  XC::profilematrix::get_min(void)        // get pointer to array of
//  {                                      // Locations of Diagonals
...
//##############################################################################
// profilematrix addition
XC::profilematrix XC::operator+(const profilematrix &lval, const profilematrix &rval)
  {
...
double *XC::profilematrix::operator*(const BJvector &arg)
  {
    int irow, iau, colht, jp, neq;
    neq= pc_profilematrix_rep->neq;

    if( neq != arg.rows())
      error("# rows of second mat must equal # cols of first for multiply#");
    double *result=new double [neq];
    if(!result)
      {
...
//##########################################################################
double & XC::profilematrix::mval(int row, int col) const // only for the members inside
  {                                                  // the profile !!!!!!!!!!!
...
//##########################################################################
double & XC::profilematrix::val(int row, int col)
  {
//    double zero=0.0;
...
//##########################################################################
double XC::profilematrix::cval(int row, int col) const
  {
    double zero=0.0;
    if ( row<=0 && row>=dimension_of_profile_M() && col>=dimension_of_profile_M() )
      {
...
//tempoutprofilematrix & XC::profilematrix::AssembleBricksInProfMatrix(stiffness_matrix & Ke,
//tempout                                                          Brick3D          * b3d,
//tempout                                                          Node             * node,
...
//tempout                if (JJ > 0)
//tempout                  {
//tempout                    MJ= pc_profilematrix_rep->jp[JJ - 1];//new_ variable corresponding to MI
//tempout                    IJ= II - JJ;
//tempout                    
...
void XC::profilematrix::profile_jp_print( void )
 {
   ::printf("\n jp \n\n " );
...
void XC::profilematrix::profile_al_print( void )
{ ::printf("\n al \n\n " );
...
void XC::profilematrix::profile_ad_print( void )
{ ::printf("\n al \n\n " );
 for (int count05= 0 ; count05 <pc_profilematrix_rep->neq ; count05++)
      {
...
//##########################################################################
void XC::profilematrix::error(const std::string &msg1,const std::string &msg2) const
  {
    std::cerr << "profmatrix error: " << msg1
              << " " << msg2 << std::endl;
    exit(1);
  }
...
//    void XC::profilematrix::rank(int profilematrix_rank)
//    {
//      this->pc_profilematrix_rep->profilematrix_rank= profilematrix_rank;
//    }
//
    long int XC::profilematrix::total_number(void) const
    {
      return this->pc_profilematrix_rep->total_numb;
    }

    void XC::profilematrix::total_number(int number)
    {
      this->pc_profilematrix_rep->total_numb= number;
    }

//    int * XC::profilematrix::dim(void) const
//    {
//      return this->pc_profilematrix_rep->dim;
//    }
//
//    int & XC::profilematrix::get_dim_pointer(void) const
//    {
...
//    }
//
    int XC::profilematrix::reference_count(int up_down)
    {
      this->pc_profilematrix_rep->n += up_down;
      return(this->pc_profilematrix_rep->n);
    }

    void XC::profilematrix::set_reference_count(int ref_count)
    {
...
// #######################
// memory manager part
// overloading operator new_ in XC::profilematrix::profilematrix_rep class  ##################
void * XC::profilematrix_rep::operator new(size_t s)
...
// overloading operator delete in XC::profilematrix::profilematrix_rep class  ##################
void XC::profilematrix_rep::operator delete(void *p)
  {                                       // see C++ reference manual by
...
XC::profilematrix & XC::profilematrix::datri(void)
  {
//  c
...
//            if(std::abs(ad(j)).lt.tol*abs(dd))  write(iow,2000) j
...
//              if(std::abs(ad(j)).lt.tol*daval)   write(iow,2003) j
...
// equations by a profile solver" in Eng. Comp. 1985 Vol 2
// December pages: 344-350.
double * XC::profilematrix::dasol(double * b)
  {
//c
...
//      daval= 0.0d0
//      do 100 j= 1,jh
//         daval= daval + std::abs(au(j))
//100   continue
//      return
//      end
double XC::profilematrix::datest(double* au, int jh)
  {
    double daval= 0.0;
    for ( int j=1 ; j<=jh ; j++ )
      daval += fabs(au[j-1]);
...
double XC::profilematrix::dot(double* a, double* b, int n)
  {
    double dot= 0.0;
    for ( int i=1 ; i<=n ; i++ )
      dot += a[i-1]*b[i-1];
...
//..      return
//..      end
void XC::profilematrix::dredu(double* al,
                          double* au,
                          double* ad,
                          int jh,
                          char flag,
...
//   10 continue
//      end
void XC::profilematrix::saxpb(double* a,
                          double* b,
                          double x,
                          int jh,
                          double* c)

...
/*  CHECK TO MAKE SURE THAT THE NODE STILL EXISTS */
    if (lxn[(*n1 << 2) + 1] <= 0) {
	*err = TRUE_;
	mesage_("** PROBLEMS IN TUCK - N1 DOES NOT EXIST **", (ftnlen)42);
	goto L290;
    }
...
/* L100: */
    ...
    mesage_("** PROBLEMS IN TUCK FINDING L5 AND NC2 **", (ftnlen)41);
    *err = TRUE_;
    goto L290;
L110:
/*  FIND L4 AND NC1 */
...
/* L120: */
    ...
    mesage_("** PROBLEMS IN TUCK FINDING L4 AND NC1 **", (ftnlen)41);
    *err = TRUE_;
    goto L290;
L130:
/*  FIND L3 */
...
/* L140: */
    ...
    mesage_("** PROBLEMS IN TUCK FINDING L3 **", (ftnlen)33);
    *err = TRUE_;
    goto L290;
L150:
/*  FIND THE ELEMENT KL5 */
	...
	mesage_("** PROBLEMS IN TUCK FINDING KL5 **", (ftnlen)34);
	*err = TRUE_;
	goto L290;
    }
...
/*  NOW FIX THE KXL ARRAY FOR LINE L3 HAVING KL5 INSTEAD OF KOLD */
	...
	mesage_("** PROBLEMS IN TUCK REPLACING KOLD FOR L3 **", (ftnlen)44);
	*err = TRUE_;
	goto L290;
    }
/*  NOW FIX THE KXL ARRAY FOR LINE L3 HAVING KL5 INSTEAD OF KOLD */
	...
	mesage_("** PROBLEMS IN TUCK REPLACING KOLD FOR L4 **", (ftnlen)44);
	*err = TRUE_;
	goto L290;
    }
...
/* L160: */
    ...
    mesage_("** PROBLEMS IN TUCK FINDING L5 IN KL5 **", (ftnlen)40);
    *err = TRUE_;
    goto L290;
L170:
...
/* L180: */
    ...
    mesage_("** PROBLEMS IN TUCK FINDING L2 IN KL2 **", (ftnlen)40);
    *err = TRUE_;
    goto L290;
L190:
/*  RECONNECT ALL LINES CONNECTED TO N1 TO NC1 EXCEPT L5 AND L2 */
    ...
    if (*err) {
	mesage_("** PROBLEMS IN TUCK GETTING N1 LINES **", (ftnlen)39);
	goto L290;
    }
    if (*graph) {
...
/*  UNHOOK L1, L2 AND L5 FROM N1 */
    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, n1, &l1, nnn, err, 
	    noroom);
    if (*noroom || *err) {
	mesage_("** PROBLEMS IN TUCK UNHOOKING L1 FROM N1 **", (ftnlen)43);
	goto L290;
    }
    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, n1, &l2, nnn, err, 
	    noroom);
    if (*noroom || *err) {
	mesage_("** PROBLEMS IN TUCK UNHOOKING L2 FROM N1 **", (ftnlen)43);
	goto L290;
    }
    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, n1, &l5, nnn, err, 
	    noroom);
    if (*noroom || *err) {
	mesage_("** PROBLEMS IN TUCK UNHOOKING L5 FROM N1 **", (ftnlen)43);
	goto L290;
    }
/*  ADD ALL LINES STILL HOOKED TO N1 TO THE LIST OF LINES FOR NC1 */
	...
	if (ll != l2 && ll != l5 && ll != l1) {
	    addlxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &nc1, &ll, nnn, 
		    err, noroom);
	    if (*noroom || *err) {
		mesage_("** PROBLEMS IN TUCK HOOKING N1' LINES TO NC1 **", (
			ftnlen)47);
		goto L290;
	    }
	}
...
/*  RECONNECT ALL LINES CONNECTED TO N2 TO N0 (EXCEPT L2) */
    ...
    if (*err) {
	mesage_("** PROBLEMS IN TUCK GETTING N2 LINES **", (ftnlen)39);
	goto L290;
    }
    if (*graph) {
	lcolor_("BLACK", (ftnlen)5);
...
/*  UNHOOK L2 FROM N2, L1 FROM N0, AND L5 FROM NC2 */
    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &n2, &l2, nnn, err, 
	    noroom);
    if (*noroom || *err) {
	mesage_("** PROBLEMS IN TUCK UNHOOKING L2 FROM N2 **", (ftnlen)43);
	goto L290;
    }
    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &n0, &l1, nnn, err, 
	    noroom);
    if (*noroom || *err) {
	mesage_("** PROBLEMS IN TUCK UNHOOKING L1 FROM N0 **", (ftnlen)43);
	goto L290;
    }
...
/*  ADD ALL LINES STILL HOOKED TO N2 TO THE LIST OF LINES FOR N0 */
	...
	if (ll != l2) {
	    addlxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &n0, &ll, nnn, 
		    err, noroom);
	    if (*noroom || *err) {
		mesage_("** PROBLEMS IN TUCK HOOKING N2' LINES TO N0 **", (
			ftnlen)46);
		goto L290;
	    }
	}

...
/*  CHECK FOR COMPLETION */
	...
	mesage_("** PROBLEMS WITH LOOP CLOSING IN PCROSS **", (ftnlen)42);
	*err = TRUE_;
	goto L140;
    }
...
L130:
	...
	if (n1test == linkpr[iperim * 3 + 1]) {
	    ...
	    mesage_("** PROBLEMS IN PCROSS WITH UNCLOSED PERIMETER **", (
		    ftnlen)48);
	    *err = TRUE_;
	    goto L140;
	}
...
/*  NOW TRY TO PINCH THE CONNECTION */
    ...
    pinch_(mxnd, mxcorn, mln, &nuid[1], &xn[1], &yn[1], &zn[1], &lxk[5], &kxl[
	    ...
	    lcorn[1], ncorn, node1, node2, graph, video, kreg, noroom, err, (
	    ftnlen)3);
    if (*noroom || *err) {
	goto L140;
    }

...
C                         ECHOUT, EIGVNS, EIGVSS, EQRT1S, ITERM,    
...
C       
      INTEGER IB1,IB2,IB3,IB4,IB5,IDGTS,IER,IERPER,ITMAX1,LOOP,N,NB,
     *   N3       
...
C       
      REAL BDELNM,BETAB,CME,DELNNM,DELSNM,FF,GAMMA,OMEGA,QA,QT,RHO,RRR, 
     *   SIGE,SME,SPECR,SPR,SRELPR,STPTST,UDNM,ZETA       
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      IF (ZETA.GE.TEMP) GO TO 50      
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE JCG'/' ','    RPARM(1) =',E10.3,    
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
   50 CONTINUE    
      TIME1 = RPARM(9)      
...
C                        ECHOUT, ITERM,  ITJSI, IVFILL, PAR 
...
C     SPECIFICATIONS FOR LOCAL VARIABLES
C       
      INTEGER IB1,IB2,IB3,ICNT,IDGTS,IER,IERPER,ITMAX1,LOOP,N,NB,   
     *   N3       
      REAL DIGIT1,DIGIT2,TEMP,TIME1,TIME2,TOL   
C       
C *** BEGIN: ITPACK COMMON  
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE JSI'/' ','    RPARM(1) =',E10.3,    
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE SOR'/' ','    RPARM(1) =',E10.3,    
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C                         PARCON, PBETA, PBSOR, PERMAT, PERROR, PERVEC, 
C                         PFSOR, PJAC, PMULT, PRBNDX, PSTOP, PVTBV, 
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE SSORCG'/' ','    RPARM(1) =',E10.3, 
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C       
  230 CONTINUE    
      ...
      BETNEW = PBETA(N,IA,JA,A,WKSP(IB1),WKSP(IB2),WKSP(IB3))/FLOAT(N)
      BETAB = AMAX1(BETAB,.25E0,BETNEW) 
  240 CALL OMEG (0.E0,1)    
      IS = 0      
C       
C ... INITIALIZE FORWARD PSEUDO-RESIDUAL
C       
  250 CALL SCOPY (N,RHS,1,WKSP(IB1),1)
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE SSORSI'/' ','    RPARM(1) =',E10.3, 
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE RSCG'/' ','    RPARM(1) =',E10.3,   
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C       
  270 CONTINUE    
      ...
      U(1) = RHS(1) 
      GO TO 330   
  280 CALL SCOPY (NR,RHS,1,WKSP(IB1),1) 
...
C     SPECIFICATIONS FOR LOCAL VARIABLES
C       
      INTEGER IB1,IB2,IDGTS,IER,IERPER,ITMAX1,JB3,LOOP,N,NB,NR,     
     *   NRP1,N3  
      REAL DIGIT1,DIGIT2,TEMP,TIME1,TIME2,TOL   
C       
C *** BEGIN: ITPACK COMMON  
...
C       
   ...
   30 TEMP = 5.0E2*SRELPR   
      ...
      IF (LEVEL.GE.1) WRITE (NOUT,40) ZETA,SRELPR,TEMP    
   40 FORMAT ('0','*** W A R N I N G ************'/'0',   
     *   '    IN ITPACK ROUTINE RSSI'/' ','    RPARM(1) =',E10.3,   
     *   ' (ZETA)'/' ','    A VALUE THIS SMALL MAY HINDER CONVERGENCE '/
     *   ' ','    SINCE MACHINE PRECISION SRELPR =',E10.3/' ',      
     *   '    ZETA RESET TO ',E10.3)  
      ZETA = TEMP 
...
C       
   30 CONTINUE    
      ...
      IF (DELNNM.EQ.0.E0) GO TO 40    
      IF (IN.NE.0) SPECR = DELNNM/DELSNM
      IF (IP.LT.IPHAT) GO TO 70       
C       
C ... STOPPING TEST, SET H  
C       
      IF (SPECR.GE.1.E0) GO TO 70     
      IF (.NOT.(SPECR.GT.(OMEGA-1.E0))) GO TO 40
      H = SPECR   
      GO TO 50    
   40 ISS = ISS+1 
      H = OMEGA-1.E0
...
C       
      ...
      IF (OMEGA.GT.1.E0) GO TO 60     
      CME = SQRT(ABS(SPECR))
      OMEGAP = 2.E0/(1.E0+SQRT(ABS(1.E0-SPECR)))
      CHANGE = .TRUE.       
      GO TO 70    
   60 IF (ISS.NE.0) GO TO 70
      IF (SPECR.LE.(OMEGA-1.E0)**FF) GO TO 70   
      IF ((SPECR+5.E-5).LE.SPCRM1) GO TO 70     
C       
C ... CHANGE PARAMETERS     
C       
...
C       
      INTEGER IA(1),JA(1),NN
      REAL A(1),RHS(NN),U(NN),U1(NN),C(NN),C1(NN),D(NN),DL(NN),WK(NN),
     *   TRI(2,1) 
C       
C ... SPECIFICATIONS FOR LOCAL VARIABLES
C       
      INTEGER N   
      REAL BETNEW,CON,DNRM,GAMOLD,RHOOLD,RHOTMP,T1,T2,T3,T4 
      LOGICAL Q1  
C       
C ... SPECIFICATIONS FOR FUNCTION SUBPROGRAMS   
C       
...
C       
      INTEGER IA(1),JA(1),NN
      REAL A(1),RHS(NN),U(NN),U1(NN),C(NN),D(NN),CTWD(NN),WK(NN)    
C       
C ... SPECIFICATIONS FOR LOCAL VARIABLES
C       
      INTEGER N   
...
C       
      RETURN      
      END 
      REAL FUNCTION CHEBY (QA,QT,RRR,IP,CME,SME)
C       
...
C       
      ...
      IF (CME.GE.1.E0) GO TO 20       
      IF (CASEII) SME = -CME
      SIGE = (CME-SME)/(2.E0-CME-SME) 
      GAMMA = 2.E0/(2.E0-CME-SME)     
      RRR = (1.E0-SQRT(ABS(1.E0-SIGE*SIGE)))/(1.E0+SQRT(ABS(1.E0-SIGE*
     *   SIGE)))  
      IS = IN     
      DELSNM = DELNNM       
      RHO = 1.E0  
...
C       
      CMOLD = CME 
      CME = AMAX1(ZM1,ZM2,CMOLD)      
      IF (CME.GE.1.E0) GO TO 40       
      SIGE = CME*CME/(2.E0-CME*CME)   
      GAMMA = 2.E0/(2.E0-CME*CME)     
      RRR = (1.E0-SQRT(ABS(1.E0-CME*CME)))/(1.E0+SQRT(ABS(1.E0-CME*CME))
     *   )
      IS = IN     
      DELSNM = DELNNM       
      RHO = 1.E0  
...
C       
   90 FORMAT (/30X,'PARAMETERS WERE CHANGED AT ITERATION NO.',I5/35X, 
     *   'SOLUTION TO CHEBYSHEV EQN.       =',E15.7/35X,  
     *   'SOLUTION TO RAYLEIGH QUOTIENT    =',E15.7/35X,  
     *   'NEW ESTIMATE FOR CME             =',E15.7/35X,  
     *   'NEW ESTIMATE FOR GAMMA           =',E15.7/35X,  
...
C       
   20 FORMAT (/30X,'ESTIMATE OF SMALLEST EIGENVALUE OF JACOBI'/37X, 
     ...
     *   'FIRST ESTIMATE OF SME            =',E15.7/35X,  
     *   'SECOND ESTIMATE OF SME           =',E15.7/35X,  
     *   'NEW ESTIMATE OF SME              =',E15.7/)     
C       
      RETURN      
      END 
      REAL FUNCTION DETERM (N,TRI,XLMDA)
C       
...
C       
  ...
  150 FORMAT (35X,'RPARM(1)  =',E15.8,4X,'(ZETA)  '/35X,'RPARM(2)  =',
     *   E15.8,4X,'(CME)   '/35X,'RPARM(3)  =',E15.8,4X,'(SME)   '/35X, 
     *   'RPARM(4)  =',E15.8,4X,'(FF)    '/35X,'RPARM(5)  =',E15.8,4X,
     *   '(OMEGA) '/35X,'RPARM(6)  =',E15.8,4X,'(SPECR) ')
      WRITE (NOUT,160) BETAB,RPARM(8),RPARM(9),RPARM(10),RPARM(11), 
     *   RPARM(12)
  160 FORMAT (35X,'RPARM(7)  =',E15.8,4X,'(BETAB) '/35X,'RPARM(8)  =',
     *   E15.8,4X,'(TOL)'/35X,'RPARM(9)  =',E15.8,4X,'(TIME1)'/35X, 
     *   'RPARM(10) =',E15.8,4X,'(TIME2)'/35X,'RPARM(11) =',E15.8,4X, 
     *   '(DIGIT1)'/35X,'RPARM(12) =',E15.8,4X,'(DIGIT2)')
C       
      RETURN      
      END 
...
C       
   ...
   10 FORMAT (///30X,'INITIAL ITERATIVE PARAMETERS',3X,   
     ...
     *   'ITMAX  =',I15,8X,'IPARM(1)'/35X,'ZETA   =',E15.8,8X,'RPARM(1)'
...
C       
   ...
   30 FORMAT (35X,'FF     =',E15.8,8X,'RPARM(4)'/35X,'CME    =',E15.8,8X
     *   ,'RPARM(2)'/35X,'SME    =',E15.8,8X,'RPARM(3)'///) 
      RETURN      
C       
C ... SSORSI      
C       
   40 WRITE (NOUT,50) PARTAD,FF,CME,OMEGA,SPECR,BETAB,BETADT
   50 FORMAT (35X,'PARTAD =',L15,8X,'IPARM(6)'/35X,'FF     =',E15.8,8X, 
     *   'RPARM(4)'/35X,'CME    =',E15.8,8X,'RPARM(2)'/35X,'OMEGA  =',
     *   E15.8,8X,'RPARM(5)'/35X,'SPECR  =',E15.8,8X,'RPARM(6)'/35X,
     *   'BETAB  =',E15.8,8X,'RPARM(7)'/35X,'BETADT =',L15,8X,'IPARM(6)'
     *   ///)     
      RETURN      
C       
C ... SSORCG      
C       
   ...
   70 FORMAT (35X,'PARTAD =',L15,8X,'IPARM(6)'/35X,'CME    =',E15.8,8X, 
     *   'RPARM(2)'/35X,'OMEGA  =',E15.8,8X,'RPARM(5)'/35X,'SPECR  =',
     *   E15.8,8X,'RPARM(6)'/35X,'BETAB  =',E15.8,8X,'RPARM(7)'/35X,
...
C       
   80 IF (ADAPT) RETURN     
      WRITE (NOUT,90) CME   
   90 FORMAT (35X,'CME    =',E15.8,8X,'RPARM(2)'///)      
C       
  100 CONTINUE    
      RETURN      
      END 
      REAL FUNCTION EIGVNS (N,TRI,D,E2,IER)     
C       
...
C       
      RETURN      
      END 
      REAL FUNCTION EIGVSS (N,TRI,START,ZETA,ITMAX,IER)   
C       
C     COMPUTES THE LARGEST EIGENVALUE OF A SYMMETRIC TRIDIAGONAL MATRIX 
C     FOR CONJUGATE GRADIENT ACCELERATION.      
...
C       
   20    IF (E2(I).EQ.0.E0) J = J+1   
         Q = 0.0E0
         IF (I.NE.N) Q = SQRT(ABS(E2(I+1)))     
         TOT = AMIN1(D(I)-P-Q,TOT)    
   30 CONTINUE    
      IF (ISW.EQ.1.AND.TOT.LT.0.0E0) GO TO 50   
      DO 40 I = 1,N 
         D(I) = D(I)-TOT    
   40 CONTINUE    
      GO TO 60    
...
C       
   ...
   80 FORMAT (4X,I5,3X,5E15.7)
      IF (LEVEL.GE.4) GO TO 250       
      RETURN      
C       
   90 WRITE (NOUT,100) IN,STPTST,RHO,GAMMA      
  100 FORMAT (4X,I5,3X,E15.7,30X,2E15.7)
      IF (LEVEL.GE.4) GO TO 250       
      RETURN      
C       
...
C       
  ...
  140 FORMAT (4X,I5,3X,5E15.7)
      IF (LEVEL.GE.4) GO TO 250       
      RETURN      
C       
  150 WRITE (NOUT,160) IN,STPTST,RHO  
  160 FORMAT (4X,I5,3X,E15.7,30X,E15.7) 
      IF (LEVEL.GE.4) GO TO 250       
      RETURN      
C       
...
C       
      ZM1 = ((1.E0-SPR)*(1.E0+BETAB*OMEGA**2)-OMEGA*(2.E0-OMEGA))/(OMEGA
     *   *(OMEGA-1.E0-SPR)) 
C       
...
C       
   ...
   20 FORMAT (/30X,'PARAMETERS WERE CHANGED AT ITERATION NO.',I5/35X, 
     *   'NEW ESTIMATE OF BETAB            =',E15.7/35X,  
     *   'SOLUTION TO CHEBYSHEV EQN.       =',E15.7/35X,  
     *   'SOLUTION TO RAYLEIGH QUOTIENT    =',E15.7/35X,  
     *   'NEW ESTIMATE FOR CME             =',E15.7/35X,  
     *   'NEW ESTIMATE FOR OMEGA           =',E15.7/35X,  
     *   'NEW ESTIMATE FOR SPECTRAL RADIUS =',E15.7/)     
C       
      RETURN      
C       
...
C       
      IF (IBMTH.LE.2) GAMMA = 1.E0/(1.E0-DTNRM/DELNNM)    
C       
C ... FOR SSOR CG 
C       
...
C       
      ...
      GO TO 20    
   10 RHO = 1.E0/(1.E0-GAMMA*DELNNM/(GAMOLD*DELSNM*RHOOLD)) 
C       
...
C       
      RETURN      
      END 
      REAL FUNCTION PBETA (NN,IA,JA,A,V,W1,W2)  
C       
...
C       
      ...
      REAL A(1),RHS(NN),U(NN),W(NN),DIGTT1,DIGTT2 
C       
C ... SPECIFICATIONS FOR LOCAL VARIABLES
C       
      INTEGER IDGTS,N       
...
C       
   20 TR = SQRT(UDNM)       
      TL = 1.E0   
      IF (CON.EQ.1.E0) GO TO 40       
      IF (IFLAG.EQ.2) GO TO 30
      TL = SQRT(DNRM)       
      TR = TR*(1.E0-CON)    
      GO TO 40    
   30 TL = SQRT(2.E0*DNRM)  
      TR = TR*(1.E0-CON*CON)
   40 STPTST = TL/TR
...
C       
      RETURN      
      END 
      REAL FUNCTION PVTBV (N,IA,JA,A,V) 
C       
...
C       
      END 
      SUBROUTINE SAXPY (NN,SAA,SX,INCXX,SY,INCYY) 
C       
...
C       
      ...
      IF (INCX.EQ.INCY) IF (INCX-1) 10 , 30 , 50
   10 CONTINUE    
C       
...
C       
         IF (RW(II).NE.0.0E0) GO TO 60
         IF (1.0E0/DI.LE.TOL) GO TO 70
         GO TO 80 
...
C       
      ...
      DO 80 II = 1,N
         ...
         DO 40 JJ = IBGN,IEND 
            IF (JA(JJ).NE.II) GO TO 40
            DI = A(JJ)      
            IF (DI.GT.0.E0) GO TO 70  
            IF (DI.EQ.0.E0) GO TO 20  
            IER = 401       
...
C       
      RETURN      
      END 
      SUBROUTINE SCOPY (NN,SX,INCXX,SY,INCYY)   
C       
...
C       
   ...
   60 CONTINUE    
      RETURN      
      END 
      REAL FUNCTION SDOT (NN,SX,INCXX,SY,INCYY) 
C       
...
C       
      REAL T(8)   
C       
      DATA T(1),T(2),T(3),T(4),T(5),T(6),T(7),T(8) / 1.5E0,1.8E0,1.85E0,
     *   1.9E0,1.94E0,1.96E0,1.975E0,1.985E0 /  
C       
      TAU = 1.992E0 
      IF (II.LE.8) TAU = T(II)
C       
...
C       
      INTEGER IC,MAXFN      
      REAL ZERO,HALF,ONE,THREE,TEN,A,B,T,FA,FB,C,FC,D,E,TOL,RM,S,P,Q,R, 
     *   RONE,TEMP

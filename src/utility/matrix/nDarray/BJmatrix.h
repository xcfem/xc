...
class BJmatrix : public nDarray
  {
    ...
    private:
      void error(const std::string &msg1,const std::string &msg2 = ""); // private function
 ...
 //special for vector
      ...
      BJmatrix(const std::string &matfile,const std::string &outfile); // read from a flat BJmatrix file
                                            // and write test output to another mat file
      BJmatrix(const BJmatrix &x);  // copy-initializer
      explicit BJmatrix(const nDarray &x);

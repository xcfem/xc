...
XC::straintensor::straintensor(int rank_of_tensor, double initval)
  : BJtensor(rank_of_tensor, def_dim_2, initval)
    {   } // default constructor

//! @brief Constructor.
XC::straintensor::straintensor(double *values)
...
//! @brief Constructor.
XC::straintensor::straintensor(const XC::BJtensor & x)
...
double XC::straintensor::Iinvariant1()const       // Structural Engineers"
  {
    return (cval(1,1)+cval(2,2)+cval(3,3));
  }

//##############################################################################
double XC::straintensor::Iinvariant2() const
  {
...
//##############################################################################
double XC::straintensor::Iinvariant3() const
  {
...
double XC::straintensor::Jinvariant1() const
  {
    return (0.0);
  }

//##############################################################################
double XC::straintensor::Jinvariant2() const
  {
...
//##############################################################################
double XC::straintensor::Jinvariant3() const
  {
...
//##############################################################################
XC::straintensor XC::straintensor::principal() const
  {
...
//##############################################################################
double XC::straintensor::sigma_octahedral() const  // Chen XC::W.F. "plasticity for
  {                                            // Structural Engineers"
    return ( this->Iinvariant1()/3.0 );        // page 59-60
  }

//##############################################################################
double XC::straintensor::tau_octahedral() const    // Chen XC::W.F. "plasticity for
  {                                             // Structural Engineers"
...
//##############################################################################
double XC::straintensor::ksi()  const                     // Chen XC::W.F. "plasticity for
  {                                            // Structural Engineers"
...
//##############################################################################
double XC::straintensor::ro() const                        // Chen XC::W.F. "plasticity for
  {                                             // Structural Engineers"
...
//##############################################################################
double XC::straintensor::p_hydrostatic() const         // Desai "Constitutive Laws
...
//##############################################################################
double XC::straintensor::q_deviatoric() const        // Desai "Constitutive Laws
  {                                       // for Engineering Materials"
...
::fprintf(stdout," double XC::straintensor::q_deviatoric() const\a\a\n");
::fprintf(stderr,"tempsqrt < 0.0 || fabs(tempsqrt) < EPS in ");
::fprintf(stderr," double XC::straintensor::q_deviatoric() const \a\a\n");
        ::exit(1);
      }
...
//##############################################################################
double XC::straintensor::theta()  const             // Chen XC::W.F. "plasticity for
  {                                             // Structural Engineers"
                                                // page 70
...
//    if ( tempabs2 < 0.001 ) return ( PI / 3.0 );
    ...
    if ( temp7>1.0 || temp7<-1.0 )
      {
        ::fprintf(stderr,"\a\n something is wrong in XC::straintensor::theta() (temp7>1.0||temp7<-1.0)\n");
...
//##############################################################################
double XC::straintensor::thetaPI() const
  {

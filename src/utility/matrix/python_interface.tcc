...
class_<XC::ID, bases<CommandEntity> >("ID")
  ...
  .def("getList", &XC::ID::getPyList, "Return the vector components in a Python list.")
  .def("index", &XC::ID::getLocation, "Returns the position of the given value in the vector.")
  // .def(self + self)
  // .def(self - self)
  // .def(self += self)
  // .def(self -= self)
...
class_<XC::Vector, bases<CommandEntity> >("Vector")
  ...
  .def(init<Vector2d>())
  .def(init<Vector3d>())
  .def(init<const int &, const double &>())
...
//  .def( "__getitem__", getItemVector, boost::python::arg( "index" ), boost::python::return_internal_reference<>() )
  ...
  .def("Norm2",&XC::Vector::Norm2,"Returns the square of the vector norm.")
  .def("size",&XC::Vector::Size)
  .def("__len__",&XC::Vector::Size,"return the length of the vector.")
  .def("getList", &XC::Vector::getPyList, "Return the vector components in a Python list.")
  ...
  .def("normalize",&XC::Vector::Normalized,"Normalize this vector using the euclidean norm.")
  .def("Normalized",&XC::Vector::Normalized,"Returns normalized vector (euclidean norm).")
//.def("assemble", &XC::Vector::Assemble, "assemble(V:XC::Vector, l:XC::ID, fact:float) Assemble the given vector into this one using the specified positions.")
  .def("Zero", &XC::Vector::Zero,"Set to zero all the components of the vector.")
  ;
...
class_<XC::Matrix, bases<CommandEntity> >("Matrix")
  ...
  .def(self_ns::str(self_ns::self))
  .def("getList", &XC::Matrix::getPyList, "Return the matrix components in a Python list of lists.")
  ...
  .def("Zero", &XC::Matrix::Zero,"Set to zero all the elements of the matrix.")
   ;

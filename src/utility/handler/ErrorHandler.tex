...
\noindent {\bf Files}
\indent \#include \f$<\tilde{ }\f$/handler/ErrorHandler.h\f$>\f$
...
//! methods warning()} and {\em fatal()}. {\em warning() is a method
//! which is called by an object when an error has occured but the program
//! can continue running after the error has been logged; fatal() is
...
\indent // Destructor
\indent {\em virtual \f$\tilde{ }\f$ErrorHandler();}
...
//! argument are converted for output. As for printf(), the format
...
//! va\_list args);} 
//! Will print to the OPS_Stream \p theStream the error message, where
\p msg is the format string  and {\em va\_list} is the list created
//! from the varying number of arguments passed to the warning() or
//! fatal() methods. It is up to the calling object to both create
//! va\_start()} and destroy {\em va\_end() this list. 

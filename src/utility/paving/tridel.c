	...
	logical *video, logical *noroom, logical *err, ftnlen dev1_len)
{
    ...
    extern /* Subroutine */ int mesage_(char *, ftnlen), adjtri_(integer *, 
    ...
    extern /* Subroutine */ int filsmo_(integer *, integer *, real *, real *, 
	    ...
	    integer *, integer *, integer *, logical *), lcolor_(char *, 
	    ftnlen), snapit_(integer *), sflush_(), nxkord_(integer *, 
...
/*  SEE IF A 2-LINE NODE NEEDS DELETED */
		...
		if (done) {
			...
			rplotl_(mxnd, &xn[1], &yn[1], &zn[1], &nxl[3], xmin, 
				xmax, ymin, ymax, zmin, zmax, lll, dev1, kreg,
				 (ftnlen)3);
			snapit_(&c__3);
		    }
...
/*  K(3) IS BETWEEN L3 & L1 */
		...
		*err = TRUE_;
		mesage_("** PROBLEMS IN TRIDEL FINDING K(1) **", (ftnlen)37);
		goto L180;
	    }
		...
		*err = TRUE_;
		mesage_("** PROBLEMS IN TRIDEL FINDING K(2) **", (ftnlen)37);
		goto L180;
	    }
		...
		*err = TRUE_;
		mesage_("** PROBLEMS IN TRIDEL FINDING K(3) **", (ftnlen)37);
		goto L180;
	    }
...
/*  THIS ELEMENT SHOULD BE DELETED. */
		...
		if (i__ != nodes[0] && i__ != nodes[1] && i__ != nodes[2] && 
			i__ != nodes[3]) {
		    mesage_("** PROBLEMS IN TRIDEL - I IS NOT IN ELEMENT K **"
			    , (ftnlen)48);
		    *err = TRUE_;
		    goto L180;
		}

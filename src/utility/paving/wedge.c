...
/* L100: */
	...
	mesage_("** PROBLEMS IN WEDGE FINDING L3 AND N4 **", (ftnlen)41);
	*err = TRUE_;
	goto L200;
L110:
/*  FIND THE ELEMENT KL3 - THE ELEMENT ON THE OTHER SIDE OF L3 */
	if (kxl[(l3 << 1) + 1] == kl1) {
	    ...
	    mesage_("** PROBLEMS IN WEDGE FINDING KL3 **", (ftnlen)35);
	    *err = TRUE_;
	    goto L200;
	}
    }
/*  GET ALL THE N1 LINES */
    ...
    if (*err) {
	mesage_("** PROBLEMS IN WEDGE GETTING N1 LINES **", (ftnlen)40);
	goto L200;
    }
/*  ERASE THE LINES */
    if (*graph || *video) {
...
/*  CORRECT ENDPOINT */
	...
	if (ll != l1 && ll != l3 && ll != l4 && ll != l6) {
	    dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, n1, &ll, nnn, 
		    err, noroom);
	    if (*noroom || *err) {
		mesage_("** PROBLEMS IN WEDGE UNHOOKING LL FROM N1 **", (
			ftnlen)44);
		goto L200;
	    }
	    addlxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &n5, &ll, nnn, 
		    err, noroom);
	    if (*noroom || *err) {
		mesage_("** PROBLEMS IN WEDGE HOOKING LL TO N5 **", (ftnlen)
			40);
		goto L200;
	    }
	    if (nxl[(ll << 1) + 1] == *n1) {
		...
		mesage_("** PROBLEMS IN WEDGE CHANGING LL ENDPOINT FROM N1 T\
O N5 **", (ftnlen)58);
		*err = TRUE_;
		goto L200;
	    }
...
/*  NOW FIX THE KXL ARRAY FOR LINE L3 */
	...
	if (kxl[(l3 << 1) + 1] == kl3) {
	    ...
	    mesage_("** PROBLEMS IN WEDGE REPLACING KL3 FOR L3 **", (ftnlen)
		    44);
	    *err = TRUE_;
	    goto L200;
	}
...
/* L150: */
	...
	mesage_("** PROBLEMS IN WEDGE REPLACING L2 WITH L4 IN KL1 **", (
		ftnlen)51);
	*err = TRUE_;
	goto L200;
L160:
...
/* L170: */
	...
	mesage_("** PROBLEMS IN WEDGE REPLACING L3 WITH L4 IN KL3 **", (
		ftnlen)51);
	*err = TRUE_;
	goto L200;
L180:

	...
	logical *err, ftnlen dev1_len)
{
    ...
    extern /* Subroutine */ int mesage_(char *, ftnlen), bpinch_(integer *, 
...
/* L150: */
		    ...
		    mesage_("** PROBLEMS IN PINCH FIXING ELEMENT **", (ftnlen)
			    38);
		    *err = TRUE_;
		    goto L210;
L160:
...
/*  RECONNECT ALL LINES CONNECTING TO NGONE TO NTHERE */
		...
		if (*err) {
		    mesage_("** PROBLEMS IN PINCH GETTING NGONELINES **", (
			    ftnlen)42);
		    goto L210;
		}
		i__1 = nl;
...
/*  UNHOOK LOLD FROM NGONE AND FROM N1 */
		dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &ngone, &
			lold, nnn, err, noroom);
		if (*noroom || *err) {
		    mesage_("** PROBLEMS IN PINCH DELETING LOLD FROM NGONE **"
			    , (ftnlen)48);
		    goto L210;
		}
		dellxn_(mxnd, &lxn[5], &nuid[1], navail, iavail, &n1, &lold, 
			nnn, err, noroom);
		if (*noroom || *err) {
		    mesage_("** PROBLEMS IN PINCH DELETING LOLD FROM N1 **", (
			    ftnlen)45);
		    goto L210;
		}
/*  ADD ALL LINES STILL HOOKED TO NGONE TO THE LIST OF LINES FOR NTHERE */
		...
		for (ii = 1; ii <= i__1; ++ii) {
			...
			if (*noroom || *err) {
			    mesage_("** PROBLEMS IN PINCH ADDINGLL TO NTHERE\
 **", (ftnlen)42);
			    goto L210;
			}
		    }

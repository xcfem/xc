...
//Constructor 00----------------------------------------------------------------------------------
XC::FiniteDeformationEP3D::FiniteDeformationEP3D()
:XC::NDMaterial(0, 0)
{
    fde3d = 0;
    fdy = 0;
...
:XC::NDMaterial(tag, ND_TAG_FiniteDeformationEP3D)
{
    if(fde3d_in)
      ...
      std::cerr << "FiniteDeformationEP3D:: FiniteDeformationEP3D failed to construct the fdElastic3D\n";
      exit(-1);
    }
    ...
    if(fdy_in)
      ...
      std::cerr << "FiniteDeformationEP3D:: FiniteDeformationEP3D failed to construct the XC::fdYield\n";
      exit(-1);
    }                  
    ...
    if(fdf_in)
      ...
      std::cerr << "FiniteDeformationEP3D:: FiniteDeformationEP3D failed to construct the XC::fdFlow\n";
      exit(-1);
    }
...
XC::NDMaterial* XC::FiniteDeformationEP3D::getCopy(const std::string &code) const
  {
    ...
    if( (code==strTipoFiniteDeformationEP3D)
      ...
      std::cerr << "FiniteDeformationEP3D::getCopy fainled:" << code << "\n";
    return tmp;
  }

...
XC::BarSlipMaterial::BarSlipMaterial(int tag,
...
XC::BarSlipMaterial::BarSlipMaterial(void):
...
void XC::BarSlipMaterial::getBondStrength(void)
  {
    if(fc <= 0.0)
      std::cerr << "WARNING : BAR-SLIP -- fc should be positive entry" << std::endl;
    ...
    if(unit == 1)   //  "mpa" (N/sq. mm) system being used
    ...
    else if(unit == 3)  // "Pa" (N/sq. m) system being used
      {
        if(bsflag == 1)   // bond strength -- weak
          {
            tauYT = 1.58*pow_fc_05;
            tauET = 56.92*pow_fc_05;
            tauEC = 69.57*pow_fc_05;
            tauYC = 117*pow_fc_05;
            tauR =  4.74*pow_fc_05;
          }
        else if(bsflag == 0)  // bond strength -- strong
          {
            tauYT = 12.65*pow_fc_05;
            tauET = 56.92*pow_fc_05;
            tauEC = 69.57*pow_fc_05;
            tauYC = 117*pow_fc_05;
            tauR = 4.74*pow_fc_05;
    ...
    else if(unit == 5)  // "ksi" (kilo-lb/sq.inch) system being used
      {
        if(bsflag == 1)   // bond strength -- weak
          {
            tauYT = 0.02*pow_fc_05;
            tauET = 0.66*pow_fc_05;
            tauEC = 0.82*pow_fc_05;
            tauYC = 1.36*pow_fc_05;
            tauR =  0.06*pow_fc_05;
          }
        else if(bsflag == 0)  // bond strength -- strong
          {
            tauYT = 0.15*pow_fc_05;
            tauET = 0.66*pow_fc_05;
            tauEC = 0.82*pow_fc_05;
            tauYC = 1.36*pow_fc_05;
            tauR =  0.06*pow_fc_05;
    ...
    else if(unit == 6)  // "ksf" (kilo-lb/sq.ft) system being used
      {
        if(bsflag == 1)   // bond strength -- weak
          {
            tauYT = 0.24*pow_fc_05;
            tauET = 7.92*pow_fc_05;
            tauEC = 9.84*pow_fc_05;
            tauYC = 16.32*pow_fc_05;
            tauR =  0.72*pow_fc_05;
          }
        else if(bsflag == 0)  // bond strength -- strong
          {
...
*/
    ...
    if(error)
      {
        std::cerr << "Error: -- input backbone not unique, XC::BarSlipMaterial::BarSlipMaterial"
                  << "\a";
      }
...
//*******************************************************************************
/*
int XC::BarSlipMaterial::setTrialStrain(double strain, double strainrate)
{

    return material->setTrialStrain(strain, strainrate);
}

double XC::BarSlipMaterial::getStrain(void)
{
    return material->getStrain();
}

double XC::BarSlipMaterial::getStress(void)
{
...
int XC::BarSlipMaterial::revertToLastCommit(void)
{
...
void XC::BarSlipMaterial::Print(std::ostream &s, int flag)
  {
    ...
    s << "Negative Envelope : " << eN << std::endl;
  }

//**********************************************************************************
void XC::BarSlipMaterial::SetEnvelope(void)
  {
...
int XC::BarSlipMaterial::setTrialStrain(double strain, double CstrainRate)
  {
    Tstate = Cstate;
    Tenergy = Cenergy;
    Tstrain = strain;
...
double XC::BarSlipMaterial::getStrain(void) const
  { return Tstrain; }
...
double XC::BarSlipMaterial::posEnvlpStress(double u)
  {
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double XC::BarSlipMaterial::posEnvlpTangent(double u)
  {
    double k = 0.0;
    int i = 0;
    while(k==0.0 && i<=4)
...
double XC::BarSlipMaterial::negEnvlpStress(double u)
  {
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double XC::BarSlipMaterial::negEnvlpTangent(double u)
  {
    double k = 0.0;
    int i = 0;
    while(k==0.0 && i<=4)
...
void XC::BarSlipMaterial::getState3(XC::Vector& state3Strain, XC::Vector& state3Stress, double kunload)
  {
        ...
        if(state3Strain(1)>state3Strain(3))
          {
            ...
            const double du = state3Strain(3) - state3Strain(0);
            const double df = state3Stress(3) - state3Stress(0);
...
void XC::BarSlipMaterial::getState4(XC::Vector& state4Strain,XC::Vector& state4Stress, double kunload)
            {
...
double XC::BarSlipMaterial::Envlp3Tangent(XC::Vector s3Strain, XC::Vector s3Stress, double u)
                    {
                            double k = 0.0;
                            int i = 0;
                            while((k==0.0||i<=2) && (i<=2))
...
double XC::BarSlipMaterial::Envlp4Tangent(XC::Vector s4Strain, XC::Vector s4Stress, double u)
                    {
                            double k = 0.0;
                            int i = 0;
                            while((k==0.0||i<=2) && (i<=2))
...
double XC::BarSlipMaterial::Envlp3Stress(XC::Vector s3Strain, XC::Vector s3Stress, double u)
                    {
                            double k = 0.0;
                            int i = 0;
                            double f = 0.0;
...
double XC::BarSlipMaterial::Envlp4Stress(XC::Vector s4Strain, XC::Vector s4Stress, double u)
                    {
                            double k = 0.0;
                            int i = 0;
                            double f = 0.0;
...
void XC::BarSlipMaterial::updateDmg(double strain)
{

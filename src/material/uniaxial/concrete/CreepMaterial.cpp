  ...
  PHI_i(0), E_i(0), DSIG_i(0), TIME_i(0), DTIME_i(0)  
{
  ...
  if (wrappedMaterial == 0) {
    opserr << "XC::CreepMaterial::CreepMaterial - failed to get copy of material" << endln;
    exit(-1);
  }  
...
XC::CreepMaterial::CreepMaterial(void):
...
double
XC::CreepMaterial::getInitialTangent(void)
{
  return Ec; //Added by AMK
}

double
XC::CreepMaterial::getCurrentTime(void)
{
...
double
XC::CreepMaterial::setCreepStrain(double time, double stress)
{
  double creep;
  double runSum = 0.0;
...
double 
XC::CreepMaterial::setPhi(double time, double tp)
{	
  // ACI Equation:
  double tmtp = time-tp;
...
double 
XC::CreepMaterial::setShrink(double time)
{
...
int
XC::CreepMaterial::setTrialStrain(double trialStrain, double strainRate)
{
...
double
XC::CreepMaterial::getStrain(void)
{
...
double
XC::CreepMaterial::getPHI_i(void)
{
  return phi_i;
}

double 
XC::CreepMaterial::getStress(void)
{
  return sig;
}

double 
XC::CreepMaterial::getTangent(void)
{
  return e;
}

double
XC::CreepMaterial::getCreep(void)
{
  return eps_cr;
}

double
XC::CreepMaterial::getShrink(void)
{
  return eps_sh;
}

double
XC::CreepMaterial::getMech(void)
{
  return eps_m;
}

int 
XC::CreepMaterial::commitState(void)
{
  iter = 0;
  ecminP = ecmin;
  ecmaxP = ecmax;
...
XC::CreepMaterial::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "XC::CreepMaterial::sendSelf -- could not send ID" << endln;
    return res;
  }
  ...
  if (res < 0) {
    opserr << "XC::CreepMaterial::sendSelf - failed to send Vector" << endln;
    return res;
  }
  
  res = wrappedMaterial->sendSelf(commitTag, theChannel);
  if (res < 0) {
    opserr << "XC::CreepMaterial::sendSelf -- could not send UniaxialMaterial" << endln;
    return res;
  }
			...
			FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "XC::CreepMaterial::recvSelf() - failed to receive data\n";
    return res;
  }
  ...
  if (theChannel.recvVector(this->getDbTag(), commitTag, data) < 0) {
    opserr << "XC::CreepMaterial::recvSelf() - failed to recvSelf\n";
    return -1;
  }
  ...
  if (wrappedMaterial == 0) {
    ...
    if (wrappedMaterial == 0) {
      opserr << "XC::CreepMaterial::recvSelf -- could not get a UniaxialMaterial" << endln;
      return -1;
    }
  }
  ...
  if (res < 0) {
    opserr << "XC::CreepMaterial::recvSelf -- count not receive Uniaxialmaterial" << endln;
    return res;
  }
...
void 
XC::CreepMaterial::Print(OPS_Stream &s, int flag)
{

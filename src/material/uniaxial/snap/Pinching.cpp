...
//
//
// Pinching.cpp: implementation of the XC::Pinching class from Fortran version.
// Originally from SNAP PROGRAM by Prof H.K. Krawinkler
//
// Written: Arash Altoontash, Gregory Deierlein, 12/01
// Revised: 05/03
//
// Purpose: This file contains the implementation for the XC::Pinching class.
//
//////////////////////////////////////////////////////////////////////
...
XC::Pinching::Pinching(int tag, XC::Vector inputParam )
  :UniaxialMaterial(tag,MAT_TAG_Pinching)
{
        if( (inputParam.Size()) < 19) 
                std::cerr << "Error: Pinching(): inputParam, size <19\n" << "\a";
    ...
    {
                std::cerr << "Error: XC::Pinching::Pinching  : All gamma values must be >= 0\n" << "\a";
    }                
        
        if ( cs < 0.0 || ck < 0.0 || ca < 0.0 || cd < 0.0 )
    {
                std::cerr << "Error: XC::Pinching::Pinching  : All 'c' values must be >= 0\n" << "\a";
    }
        
        if ( capSlope > 0.0 )
    {
                std::cerr << "Error: XC::Pinching::Pinching  : CapSlope must be < 0\n" << "\a";
    }                
        
        if ( Resfac <  0.0  || Resfac > 1.0)
    {
                std::cerr << "Error: XC::Pinching::Pinching  : Residual must be > 0 and <= 1\n" << "\a";
    }                
        
        if ( a_pinch < 0.0 || a_pinch > 1.0)
    {
                std::cerr << "Error: XC::Pinching::Pinching  : kappad (dev. point)must be > 0 and <= 1\n" << "\a";
    }
        
        if ( alpha > 0.8 || alpha < -0.8 )
    {
                std::cerr << "Error: XC::Pinching::Pinching  : alpha must be < 0.8 and > -0.8\n" << "\a";        
    }
        
        if ( alpha == capSlope )
    {
                std::cerr << "Error: XC::Pinching::Pinching  : Error: alpha Hard. can not be equal to alphaCap\n" << "\a";        
    }
...
//! @brief Constructor.
XC::Pinching::Pinching(int tag)
  :UniaxialMaterial(tag,MAT_TAG_Pinching) 
  {
        if ( DEBG ==1 ) fprintf( OutputFile , "Empty constructor\n" );        // debugging
  }

XC::Pinching::Pinching()
...
void XC::Pinching::Print(std::ostream &s, int flag)
{
...
int XC::Pinching::revertToLastCommit()
{
...
double XC::Pinching::getStrain(void) const
  { return hsTrial[0]; }
...
int XC::Pinching::sendSelf(CommParameters &cp)
{
 ...
 XC::UniaxialMaterial *XC::Pinching::getCopy(void) const
{
...
int XC::Pinching::setTrialStrain( double d, double strainRate)
{
        ...
        if ( kon == 0 )
    {
        ...
        if( flagDeg == 1 || flagDeg == 2 ) {
                ...
                if( betas>=1.0 || betak>=1.0 || betaa>=1.0) {
                        std::cerr << "Beta greater than one\n";
                        flgstop=1;
                }
...
void XC::Pinching::recordInfo(int cond )
{

}


void XC::Pinching::envelPosCap( double fy, double alphaPos, double alphaCap,
...
void XC::Pinching::envelNegCap( double fy, double alphaNeg, double alphaCap,
                            double cpDsp, double d, double *f, double *ek)
{
        double dy,Res,rcap,dres;

...
//
//
// PinchingDamage.cpp: implementation of the XC::PinchingDamage class from Fortran version.
// Originally from SNAP PROGRAM by Prof H.K. Krawinkler
//
// Written: A. Altoontash & Prof. G. Deierlein 12/01
// Revised: 05/03
//
// Purpose: This file contains the implementation for the XC::PinchingDamage class.
//
//////////////////////////////////////////////////////////////////////
...
XC::PinchingDamage::PinchingDamage(int tag, XC::Vector inputParam , DamageModel *strength, DamageModel *stiffness,DamageModel *accelerated,DamageModel *capping)
  :UniaxialMaterial(tag,MAT_TAG_SnapPinchingDamage)
{
        if( (inputParam.Size()) < 11) 
                std::cerr << "Error: PinchingDamage(): inputParam, size <19\n" << "\a";
        ...
        */
        ...
        if ( capSlope > 0.0 )
    {
                std::cerr << "Error: XC::PinchingDamage::PinchingDamage  : CapSlope must be < 0\n" << "\a";
    }                
        
        if ( Resfac <  0.0  || Resfac > 1.0)
    {
                std::cerr << "Error: XC::PinchingDamage::PinchingDamage  : Residual must be > 0 and <= 1\n" << "\a";
    }                
        
        if ( a_pinch < 0.0 || a_pinch > 1.0)
    {
                std::cerr << "Error: XC::PinchingDamage::PinchingDamage  : kappad (dev. point)must be > 0 and <= 1\n" << "\a";
    }
        
        if ( alpha > 0.8 || alpha < -0.8 )
    {
                std::cerr << "Error: XC::PinchingDamage::PinchingDamage  : alpha must be < 0.8 and > -0.8\n" << "\a";        
    }
        
        if ( alpha == capSlope )
    {
                std::cerr << "Error: XC::PinchingDamage::PinchingDamage  : Error: alpha Hard. can not be equal to alphaCap\n" << "\a";        
    }
        ...
        if ( accelerated != nullptr )
        {
                ...
                if ( AccDamage == nullptr ) {
                        std::cerr << "Error: XC::CloughDamage::CloughDamage  : Can not make a copy of accelerated stiffness degradation damage model\n" << "\a";        
                        exit(-1);
                }
        }        
...
XC::PinchingDamage::PinchingDamage(int tag)
  :UniaxialMaterial(tag,MAT_TAG_SnapPinchingDamage) 
  {
    if ( DEBG ==1 ) fprintf( OutputFile , "Empty constructor\n" );        // debugging
  }

XC::PinchingDamage::PinchingDamage()
...
int XC::PinchingDamage::revertToLastCommit()
{
...
double XC::PinchingDamage::getTangent() const
{
        if ( DEBG ==1 ) fprintf( OutputFile , "Get tangent\n" );        // debugging
        return hsTrial[2];
}

double XC::PinchingDamage::getInitialTangent (void) const
{
        return elstk;
}

double XC::PinchingDamage::getStress() const
{
        return hsTrial[1];
}
...
int XC::PinchingDamage::sendSelf(CommParameters &cp)
{
 ...
 XC::UniaxialMaterial *XC::PinchingDamage::getCopy(void) const
{
...
int XC::PinchingDamage::setTrialStrain( double d, double strainRate)
{
        ...
        if ( kon == 0 )
    {
        ...
        if( flagDeg == 1 || flagDeg == 2 ) {
                ...
                if( deltaD < 0.0) {
                        ...
                        if( StrDamage != nullptr ) betas = StrDamage->getNegDamage();
                        if( betas>=1.0 ) {
                                std::cerr << "Total loss for strength degradation\n";
                                betas = 1.0;
                        }
                        if( AccDamage != nullptr ) betaa = AccDamage->getNegDamage();
                        if( betaa>=1.0 ) {
                                std::cerr << "Total loss for accelerated stiffness degradation\n";
                                betaa = 1.0;
                        }
                        if( CapDamage != nullptr ) betad = CapDamage->getNegDamage();
                        if( betad>=1.0 ) {
                        ...
                        if( StrDamage != nullptr ) betas = StrDamage->getPosDamage();
                        if( betas>=1.0 ) {
                                std::cerr << "Total loss for strength degradation\n";
                                betas = 1.0;
                        }
                        if( AccDamage != nullptr ) betaa = AccDamage->getPosDamage();
                        if( betaa>=1.0 ) {
                                std::cerr << "Total loss for accelerated stiffness degradation\n";
                                betaa = 1.0;
                        }
                        if( CapDamage != nullptr ) betad = CapDamage->getPosDamage();
                        if( betad>=1.0 ) {
                                std::cerr << "Total loss for capping degradation\n";
                                betad = 1.0;
                        }
...
void XC::PinchingDamage::recordInfo(int cond )
{

}


void XC::PinchingDamage::envelPosCap( double fy, double alphaPos, double alphaCap,
...
void XC::PinchingDamage::envelNegCap( double fy, double alphaNeg, double alphaCap,
                            double cpDsp, double d, double *f, double *ek)
{
        double dy,Res,rcap,dres;

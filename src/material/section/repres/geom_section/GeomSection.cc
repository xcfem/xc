...
bool XC::GeomSection::procesa_comando(CmdStatus &status)
  {
    const std::string cmd= deref_cmd(status.Cmd());
    if(verborrea>2)
      std::clog << "(GeomSection) Procesando comando: " << cmd << std::endl;

    if(cmd == "regiones")
      {
        regiones.LeeCmd(status);
        return true;
      }
    else if(cmd == "armaduras")
      {
        capas_armado.LeeCmd(status);
        return true;
      }
    else if(cmd == "for_each_region")
      {
        regiones.EjecutaBloqueForEach(status,status.GetBloque());
        return true;
      }
    else if(cmd == "for_each_capa_armadura")
      {
        capas_armado.EjecutaBloqueForEach(status,status.GetBloque());
        return true;
      }
    else if(cmd == "tag_sis_ref")
      {
        tag_sis_ref= interpretaSize_t(status.GetString()); //Nuevo identificador del sistema de referencia.
        return true;
      }
    else if(cmd == "sistema_referencia") //Crea un nuevo sistema de referencia.
      {
        ...
        if(fnc_indices.size()>0)
          tag_sis_ref= convert_to_size_t(fnc_indices[0]); //Identificador del sistema de referencia.
	std::string tipo_sis_ref= "cartesianas";
        ...
        if(tag_sis_ref>0) //El 0 se reserva para el sistema de coordenadas universal.
          {
            SisRefScc *ref= creaSisRef(tipo_sis_ref);
            ref->LeeCmd(status);
          }
        else
    ...
    else if(cmd == "spot") //Crea un nuevo punto.
      {
	std::cerr << "DEPRECATED; use python" << std::endl;
        return true;
      }
    else if(cmd == "tag_spot")
      {
        tag_spot= interpretaSize_t(status.GetString()); //Nuevo identificador del punto.
        return true;
      }
    else if(cmd == "linea") //Crea una nueva línea
      {
        std::deque<boost::any> fnc_indices= status.Parser().SeparaIndices(this);
        if(fnc_indices.size()>0)
          tag_eje= convert_to_size_t(fnc_indices[0]); //Identificador de la línea.
        Eje *l= creaEje<Segment>();
        l->LeeCmd(status);
        return true;
      }
    else if(cmd == "tag_eje")
      {
        tag_eje= interpretaSize_t(status.GetString()); //Nuevo identificador del eje.
        return true;
      }
    else if(cmd == "for_each_spot")
      {
        
        const std::string nmbBlq= nombre_clase()+":for_each_spot";
        const std::string bloque= status.GetBloque();
        for(lst_spots::iterator i=spots.begin();i!=spots.end();i++)
          (*i).second->EjecutaBloque(status,bloque,nmbBlq);
        return true;
      }
    else if(cmd == "for_each_eje")
      {
        
        const std::string nmbBlq= nombre_clase()+":for_each_eje";
        const std::string bloque= status.GetBloque();
        for(lst_ejes::iterator i=ejes.begin();i!=ejes.end();i++)
          (*i).second->EjecutaBloque(status,bloque,nmbBlq);
        return true;
      }
    else
...
any_const_ptr XC::GeomSection::GetProp(const std::string &cod) const
  {
    if(cod=="regiones")
      { return any_const_ptr(&regiones); }
    else if(cod=="capas_armado")
      { return any_const_ptr(&capas_armado); }
    else if(cod=="getTagSistemaReferencia")
      return any_const_ptr(tag_sis_ref);
    else if(cod=="getTagSpot")
      return any_const_ptr(tag_spot);
    else if(cod=="dist_spots")
      {
        ...
        tmp_gp_dbl= DistSpots(iPuntoA,iPuntoB);
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="getRecubrimiento")
      {
        const Pos2d p= popPos2d(cod);
        tmp_gp_dbl= getRecubrimiento(p);
        return any_const_ptr(tmp_gp_dbl);
      }
    else if(cod=="getContornoRegiones")
      {
        tmp_gp_pol2d= getContornoRegiones();

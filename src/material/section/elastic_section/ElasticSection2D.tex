...
of $alpha$. The integers \p tag and MAT\_TAG\_ElasticSection2D, defined in
...
using the same values of \p tag, $E$, $A$, $I$, $G$, and $\alpha.  
...
\indent {\em    int sendSelf(int commitTag, Channel \&theChannel); }\\
Creates a Vector of size $6$ into which it places  \p tag,
$E$, $A$, $I$, $G$, and $\alpha$. Invokes sendVector() on  \p theChannel using the
...
\indent {\em    int recvSelf(int commitTag, Channel \&theChannel, 
		 FEM\_ObjectBroker \&theBroker); }\\
Creates a Vector of size $6$. Invokes recvVector() on  {\em
...
Vector to set its \p tag, $E$, $A$, $I$, $G$, and $\alpha$. Returns $0$ if successful, a
warning message is printed and a negative number is returned if the Channel 
object fails to receive the Vector. \\

\indent {\em    void Print(OPS_Stream \&s, int flag =0);} \\
Prints to the stream \p s the objects \p tag, $E$, $A$, $I$, $G$, and
$\alpha$ values. \\

...
//! @brief Constructor.
XC::UniformGrid::UniformGrid(Modelador *m,const size_t &i)
...
XC::UniformGrid::UniformGrid(const std::string &nombre,Modelador *m,const size_t &i)
...
std::set<XC::SetBase *> XC::UniformGrid::get_sets(void) const
  {
    ...
    if(mdlr)
      ...
      std::cerr << "UniformGrid::get_sets; no se ha definido el modelador." << std::endl;
    return retval;
  }

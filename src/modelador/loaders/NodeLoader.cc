...
XC::NodeLoader::NodeLoader(Modelador *mdlr)
  : Loader(mdlr), ngdl_def_nodo(2),ncoo_def_nodo(3),nodo_semilla(nullptr) {}
...
XC::Node *XC::NodeLoader::duplicateNode(const int &tagNodoOrg)
  {
    ...
    if(!ptr_nodo_org)
      std::cerr << "Modelador::nuevoNodo; no se encontró el nodo de tag:"
                << tagNodoOrg << std::endl;
    else
      {

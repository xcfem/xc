...
#include "modelador/cad/entidades/Face.h"
...
//! @brief Constructor.
XC::Set::Set(const std::string &nmb,Modelador *md)
  : SetMeshComp(nmb,md) {}
...
bool XC::Set::procesa_comando(CmdStatus &status)
  {
            ...
            if(fnc_indices.size()>0)
              {
                ...
                set_base= dynamic_cast<Set *>(mdlr->get_sets().busca_set(nmbSetBase));
                if(!set_base)
                  {
	...
	const Lista tmp= interpretaLista(status.GetString());
    ...
    else if(cmd == "sel_set")
      {
        ...
        if(mdlr)
          {
            const Set *set= dynamic_cast<Set *>(mdlr->get_sets().busca_set(nmb_set));
            if(set)
              agrega_listas(*set);
            else
              {
        ...
        if(args.size()>=2)
          {
            ...
            if(mdlr)
              {
                const Set *set= dynamic_cast<Set *>(mdlr->get_sets().busca_set(nmbSet));
                if(set)

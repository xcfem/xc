...
XC::SetBase *XC::MapSet::broke_set(const std::string &nmb,const std::string &nmb_clase)
  {
    ...
    if(existe(nmb)) //El conjunto existe
      ...
      {
        if(nmb_clase == "XC::Set")
          retval= new Set(nmb,mdlr);
//         else if(nmb_clase == "XC::SetEstruct")
//           retval= new SetEstruct(nmb,mdlr);
        else
...
XC::MapSet::MapSet(Modelador *m)
  : EntCmd(m), MovableObject(0), mdlr(m),total(nullptr)
  { setup_total(); }

//! @brief Constructor de copia (NO COPIA LOS CONJUNTOS).
XC::MapSet::MapSet(const MapSet &otro)
  : EntCmd(otro), MovableObject(otro), mdlr(otro.mdlr), total(nullptr)
  { setup_total(); }

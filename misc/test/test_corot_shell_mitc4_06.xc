\c{Test de elaboración propia
 ...
 Un elemento shell corrotacional con lados unidad, nodo 1 en el origen y contenido en el
plano XY, se rota 90 grados en torno al eje paralelo al X por su centro de
gravedad moviéndose como un sólido rígido.

A continuación se le imponen a sus nodos los siguientes movimientos expresados
en coordenadas locales (-1,-1),(-1,1),(1,1),(-1,1). El movimiento total se
expresa en coordenadas globales como sigue:

   Mov. nodo 1: [-1,0.5,-1.5]
   Mov. nodo 2: [1,0.5,-1.5]
   Mov. nodo 3: [1,-0.5,1.5]
   Mov. nodo 4: [-1,-0.5,1.5]
...
\mdlr
  {
    ...
    \nodos
      {
        \nod[1]{\setTrialDisp{[-1,0.5,-1.5]}}
        \nod[2]{\setTrialDisp{[1,0.5,-1.5]}}
        \nod[3]{\setTrialDisp{[1,-0.5,1.5]}}
        \nod[4]{\setTrialDisp{[-1,-0.5,1.5]}}

        \nod[11]{\setTrialDisp{[-1,0,-1]}}
        \nod[12]{\setTrialDisp{[1,0,-1]}}
        \nod[13]{\setTrialDisp{[1,0,1]}}
        \nod[14]{\setTrialDisp{[-1,0.,1]}}
      }
    \elementos
      {
        \element[0]
...
\def_prop["ratio1","double"]{@abs(@fuerzaN1Corot-@fuerzaN1)/@abs(@fuerzaN1)}
\def_prop["ratio2","double"]{@abs(@fuerzaN2Corot-@fuerzaN2)/@abs(@fuerzaN2)}
\def_prop["ratio3","double"]{@abs(@fuerzaN3Corot-@fuerzaN3)/@abs(@fuerzaN3)}
\def_prop["ratio4","double"]{@abs(@fuerzaN4Corot-@fuerzaN4)/@abs(@fuerzaN4)}
...
\if((@ratio1<1e-15) & (@ratio2<1e-15) & (@ratio3<1e-15) & (@ratio4<1e-15) & (@ratio5<1e-15) & (@ratio6<1e-15))
  {
    \then{\print{"test corot shell MITC4 06: pasa.\n"}}
    \else{\print{"test corot shell MITC4 06: falla.\n"}}
  }
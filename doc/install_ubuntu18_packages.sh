...
apt-get install cmake cimg-dev g++ gfortran gnuplot libarpack2-dev libarpack++2-dev libcgal-dev libdb-dev libf2c2-dev libglib2.0-dev libgmp3-dev libgtk2.0-dev libgtkgl2.0-dev libgtkglextmm-x11-1.2-dev libgtkmm-2.4-dev libgts-bin libgts-dev liblapack-dev libmpfr-dev libmysql++-dev libmetis-dev libparmetis-dev libplot-dev libsqlite3-dev libsuperlu-dev libsuitesparse-dev libvtk6-dev petsc-dev tcl-dev python-vtk python-scipy python-sympy python-matplotlib python-pandas python-sklearn python-pip

#mayavi installation. Ubuntu 'mayavi' package seems to require VTK 6 so we use pip (to reconsider because we already use VTK 6 LCPT 24/09/2018)

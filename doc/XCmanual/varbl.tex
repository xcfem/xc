...
{
{\tt commitState()} & the element is to commit its current state; returns $0$ if sucessfull, a negative number if not\\
{\tt revertToLastCommit()} & the element is to set its current state to the last committed state; returnS $0$ if sucessfull, a negative number if not\\
{\tt revertToStart} & the element is to set its current state to the state it was at before the analysis started; returns $0$ if sucessfull, a negative number if not\\

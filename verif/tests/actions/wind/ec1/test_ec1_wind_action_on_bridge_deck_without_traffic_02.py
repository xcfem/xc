...
ratio3= abs(cr-1.2700762282569062)/1.2700762282569062
...
ratio4= abs(Iv-0.14959732004492532)/0.14959732004492532
...
ratio6= abs(ce-3.302295009164975)/3.302295009164975

# Depth subjected to wind pressure (see figure 1.11 of the book).
girdersDepth= 2.8
deckThickness= 0.4-0.025*2.5
concreteSupport= 0.2
safetyBarriers= 2*0.3 # See table 8.1 of EN 1991-1-4:2005.
dtot= girdersDepth+deckThickness+concreteSupport+safetyBarriers
ratio7= abs(dtot-3.9375)/3.9375
...
beta= 0.0
cfz= ec1_wind.get_bridge_deck_vertical_force_coefficient(b= deckWidth, dtotVP= dtot, alpha= alph, beta= beta)
Fwz= cscd*cfx*qp*deckWidth*deckLength
# Compute vertical pressure distribution.
vpd= ec1_wind.get_vertical_pressure_distribution(terrainCategory= terrainCategory, x0= -deckWidth/2.0, x1= deckWidth/2.0, dtotVP= dtot, z= z, vb= vb, zMax= zMax, rho= rho, k1= k1, c0= c0, alpha= alph, beta= beta)

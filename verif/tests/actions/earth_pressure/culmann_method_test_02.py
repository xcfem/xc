...
This is the same as the previous test, but using multiple points
to define the backfill profile just to thest that this works as
intended.
'''
...
backfillSlopeVDir= geom.Vector2d(math.cos(backfillSlopeAngle), math.sin(backfillSlopeAngle))
backfillPoints= list()
for i in range(0,10):
    pt= ptA+i*backfillSlopeVDir
    backfillPoints.append(pt)

...
concrete= EC2_materials.EC2Concrete("C35/45",-35e6,1.5) # page 73
...
fiRH= concrete.getCreepFiRH(RH,h0) # factor to allow for the effect of relative
                                   # humidity on the notional creep coefficient
                                   
betafcm= concrete.getCreepBetafcm() # factor to allow for the effect of
                                     # concrete strength on the notional creep
                                     # coefficient

betat0= concrete.getCreepBetat0(t0)  # factor to allow for the effect of
                                      # concrete age at loading
                                      # on the notional creep coefficient
                                      
betactt0= concrete.getCreepBetactt0(t_inf,t0,RH,h0) # coefficient to describe
                                                     # the development of creep
                                                     # with time after loading.

betaH= concrete.getCreepBetaH(RH,h0) # coefficient depending on the relative
                                      # humidity (RH) and the notional member
                                      # size (h0).
                                      
fi0=concrete.getCreepFi0(t0,RH,h0) # notional creep coefficient for the
                                    # calculation of the creep coefficient.
                            
#Creep coefficients:
fit_inf_t0= concrete.getCreepFitt0(t_inf,t0,RH,h0)

...
# Actions
N= 0.0# 500.688e3 # Axial force.
M= 288.0#-367.658 # Bending moment.
...
# for f in fibers:
#     fiberMaterial= f.getMaterial()
#     typo= str(type(fiberMaterial))
#     if('Elastic' in typo):
#         print(f.getPos(), fiberMaterial.E/1e9, f.getArea()*1e4)
#     else:
#         print(f.getPos(), fiberMaterial.Ec/1e9, f.getArea()*1e4)
...
solProc= predefined_solutions.PlainNewtonRaphson(feProblem, convergenceTestTol= 1e-3, maxNumIter= 10, printFlag= 1)
# solProc= predefined_solutions.PlainStaticModifiedNewton(feProblem, convergenceTestTol= 1e-3, maxNumIter= 10, printFlag= 1)
solProc.setup()

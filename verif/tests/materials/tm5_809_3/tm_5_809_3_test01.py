...
S= S_iu*units_utils.inchToMeter # Cell spacing (meters).
cellReinf= tm5_809_3_materials.CMUWallCellReinforcement(reinfArea= ACI_materials.num6Area)
cmuWall= tm5_809_3_materials.CMUWallFabric(thickness= 12*units_utils.inchToMeter,spacing= S, cellReinf= cellReinf)
wallHeight= 24*units_utils.footToMeter # 24 ft-> m
...
FmIU= Fm/units_utils.poundToN*units_utils.inchToMeter**2 # Imperial units.
...
ratios.append(abs(b/units_utils.inchToMeter-24)/24)
...
MrmIU= Mrm/units_utils.poundToN/units_utils.footToMeter*units_utils.inchToMeter # Imperial units
MrmIU_ref= 4147/S_iu
...
MrsIU= Mrs/units_utils.poundToN/units_utils.footToMeter*units_utils.inchToMeter # Imperial units
MrsIU_ref= 4586.1816/S_iu
ratios.append(abs(MrsIU-MrsIU_ref)/MrsIU_ref)

# Allowable axial stress.
Fa= cmuWall.getAllowableAxialStress(wallHeight)
FaIU= Fa/units_utils.poundToN*units_utils.inchToMeter**2
FaIU_ref= 211.68
ratios.append(abs(FaIU-FaIU_ref)/FaIU_ref)

# Capacity factor
effectiveArea= cmuWall.getEffectiveArea()
effectiveAreaIU= cmuWall.getEffectiveArea()/units_utils.inchToMeter**2
Mu= 1.33*cmuWall.getResistingMomentPerUnitLength()
Fu= 1.33*cmuWall.getResistingAxialForcePerUnitLength(wallHeight)
axialLoad= 38.3*units_utils.poundToN/(units_utils.inchToMeter**2)*cmuWall.getEffectiveAreaPerUnitLength()

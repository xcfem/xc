# -*- coding: utf-8 -*-
''' Check strain load representation on mixed elements. 
'''
...
n21= nodes.newNodeXYZ(0,L,L)
n22= nodes.newNodeXYZ(L,L,L)
...
trussMat= typical_materials.defElasticMaterial(preprocessor, "trussMat",E)
...
truss= modelSpace.newElement("Truss", [n21.tag,n22.tag])
truss.sectionArea= A

# Constraints
for n in [n1, n2, n3, n4, n11, n12, n21, n22]:
...
Ttop= AT # Temperature at top side (Celsius degrees)
...
shellEleLoad.setStrainComp(0,3,curvature) #(id of Gauss point, id of component, value)
shellEleLoad.setStrainComp(1,3,curvature)
shellEleLoad.setStrainComp(2,3,curvature)
shellEleLoad.setStrainComp(3,3,curvature)

beamEleLoad= lp0.newElementalLoad("beam_strain_load")
beamEleLoad.elementTags= xc.ID([elasticBeam.tag])
gradientDeformation= xc.DeformationPlane(xc.Vector([0.0, curvature, 0.0]))
beamEleLoad.backEndDeformationPlane= gradientDeformation
beamEleLoad.frontEndDeformationPlane= gradientDeformation

trussLoad= lp0.newElementalLoad("truss_strain_load")
trussLoad.elementTags= xc.ID([truss.tag])
trussLoad.eps1= alpha*AT
trussLoad.eps2= alpha*AT

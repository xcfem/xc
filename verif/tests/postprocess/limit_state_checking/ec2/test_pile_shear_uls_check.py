# -*- coding: utf-8 -*-
''' Check shear strength of circular section and compare results with those foun in the calculation report of the project:

Proyecto de construcción de plataforma de la línea de alta velocidad de la conexión Madrid – País Vasco – frontera francesa. tramo: Burgos – Vitoria. Subtramo: variante ferroviaria de Burgos - Valle de las Navas. Anejo 11. Apéndice 1. Página 2076.
'''
...
# midasVRdc= 1004.9
# midasVRds= 252.3
# midasCF= 364.52/(midasVRdc+midasVRds)
# print(midasCF)
# The value computed in the report is 0.29 but it probably does not consider the
# favourable effect of the axial load (the axial load does not appear in
# the results table).
refMeanFC0= 0.2166961857861631
ratio1= abs(meanCFs[0]-refMeanFC0)/refMeanFC0
refMeanFC1= refMeanFC0
...
# Report results.
# shearCheckResultsData= limitState.getOutputDataBaseDict()
# results= shearCheckResultsData['elementData']['9999']
# for i, key in enumerate(results):
#     sectionResults= eval(results[key])
#     print(str(9999)+', '+str(i)+', CF= '+cf.Factor.format(sectionResults.CF)+' N= '+cf.Force.format(sectionResults.N/1e3)+' kN, My= '+cf.Force.format(sectionResults.My/1e3)+' kN.m, Vy= '+cf.Force.format(sectionResults.Vz/1e3), ' kN')

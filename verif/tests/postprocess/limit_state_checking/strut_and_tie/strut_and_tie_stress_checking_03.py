# -*- coding: utf-8 -*-
''' Trivial test cheching report generation of a limit state analysis on a 
strut-and-tie model when control variables are read from a file.
'''
...
if(not pth):
  pth= "."
auxPath= pth+"/../../../aux/internal_forces/"
inputFileName= auxPath+'strut_and_tie_stress_checking_03.json'
destFileName= cfg.projectDirTree.getVerifStrutAndTieFile()
os.makedirs(os.path.dirname(destFileName))
shutil.copyfile(inputFileName, destFileName)

modelSpace.readControlVars(inputFileName=cfg.projectDirTree.getVerifStrutAndTieFile())

limitStateLabel= limitStateToCheck.label
report=rprt.ReportGenerator(modelSpace,cfg)
argsTrussEl=['CF','N']
fileNames= report.checksReport(limitStateLabel,setsTrussEl= [setCalc], argsTrussEl=argsTrussEl)

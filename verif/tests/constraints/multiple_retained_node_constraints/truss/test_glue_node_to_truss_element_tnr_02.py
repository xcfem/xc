...
solProc= predefined_solutions.FullGenNewtonRaphson(feProblem, constraintHandlerType= 'transformation', maxNumIter= 2, printFlag= 0)
solProc.setup()
...
## Displacement on node 2.
n2Disp= n2.getDisp[0]
ratio5= abs(n2Disp-refDisp)/-refDisp 
...
print("ratio4= ", ratio4)
print('node 2 displacement: ', n2Disp)
print("ratio5= ", ratio5)
'''

import os
fname= os.path.basename(__file__)
if (abs(ratio1)<1e-15) & (abs(ratio2)<1e-5) & (abs(ratio3)<1e-5) & (abs(ratio4)<1e-8) & (abs(ratio5)<1e-8):
    print('test '+fname+': ok.')
else:

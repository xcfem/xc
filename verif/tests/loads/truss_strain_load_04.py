...
eleLoad0= lp0.newElementalLoad("truss_strain_load")
eleLoad0.elementTags= xc.ID([truss.tag])
eleLoad0.eps1= alpha*AT/2.0
eleLoad0.eps2= alpha*AT/2.0

lp1= modelSpace.newLoadPattern(name= '1')
eleLoad1= lp1.newElementalLoad("truss_strain_load")
eleLoad1.elementTags= xc.ID([truss.tag])
eleLoad1.eps1= alpha*AT/2.0
eleLoad1.eps2= alpha*AT/2.0


# We add the load case to domain.
modelSpace.addLoadCaseToDomain(lp0.name)
modelSpace.addLoadCaseToDomain(lp1.name)
#print('active load patterns: ', modelSpace.getActiveLoadPatternNames())
...
result= modelSpace.analyze(calculateNodalReactions= False)
truss.getResistingForce()
ratio3= ((truss.getN()-N/2.0)/(N/2.0))
initStrain3= truss.getMaterial().initialStrain
ratio4= abs(initStrain3-alpha*AT/2.0)/(alpha*AT/2.0)
...
truss.getResistingForce()
ratio5= truss.getN()
initStrain4= truss.getMaterial().initialStrain
ratio6= abs(initStrain4)
...
print("ratio4= ",ratio4)
print("ratio5= ",ratio5)
print("initial strain 4: ", initStrain4)
print("ratio6= ",ratio6)
'''
...
if(abs(ratio1)<1e-5 and (abs(ratio2)<1e-5) and (abs(ratio3)<1e-5) and (abs(ratio4)<1e-5) and (abs(ratio5)<1e-5) and (abs(ratio6)<1e-5)):
    print('test '+fname+': ok.')
else:

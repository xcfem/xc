...
zlA= elements.newElement("ZeroLength",xc.ID([n1.tag,n2.tag]))
zlA.sectionArea= A
zlB= elements.newElement("ZeroLength",xc.ID([n1.tag,n2.tag]))
zlB.sectionArea= A
...
# Deactivate zlB element.
zlBSet= modelSpace.defSet(elements=[zlB])
modelSpace.deactivateElements(zlBSet, freezeDeadNodes= False)
...
ratio0= abs(R1a+F)/F
N1a= zlA.getMaterials()[0].getStress()
ratio1= abs(N1a-F)/F
Ux1a= n2.getDisp[0]
Ux1aRef= F/E/A
...
modelSpace.activateElements(zlBSet)
...
ratio3= abs(R1b+F)/F
N1Ab= zlA.getMaterials()[0].getStress() # Axial force in the first element.
ratio4= abs(N1Ab-F)/F
N1Bb= zlB.getMaterials()[0].getStress() # Axial force in the awakened element.
...
N1Ac= zlA.getMaterials()[0].getStress() # Axial force in the first element.
ratio8= abs(N1Ac-F*1.5)/1.5/F # Three quarters of the total load.
N1Bc= zlB.getMaterials()[0].getStress() # Axial force in the awakened element.
ratio9= abs(N1Bc-0.5*F)/0.5/F # No axial force in the awakened element.
Ux1c= n2.getDisp[0] # Horizontal displacement.
Ux1cRef= Ux1bRef+F/E/A/2 # 1.5 times the previous displacement.

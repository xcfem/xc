...
''' Check that adding an element to an existing "hot" model: a model that has
    been already analyzed gives the correct results. Check CorotTrussSection element.
'''
...
## Compute solution
solProc= predefined_solutions.PlainNewtonRaphson(feProblem, convergenceTestTol= 1e-8, printFlag= 0)
result= solProc.solve(calculateNodalReactions= True, reactionCheckTolerance= 1e-7)
...
if (abs(ratio0)<1e-8) & (abs(ratio1)<1e-8) & (abs(ratio2)<1e-8) & (abs(ratio3)<1e-8) & (abs(ratio4)<1e-8) & (abs(ratio5)<1e-8) & (abs(ratio6)<1e-8) & (abs(ratio7)<1e-8)& (abs(ratio8)<1e-8) & (abs(ratio9)<1e-8) & (abs(ratio10)<1e-8):
    print('test '+fname+': ok.')
else:

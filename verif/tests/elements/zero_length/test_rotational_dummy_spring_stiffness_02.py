...
## Define nodes.
n0= modelSpace.newNodeXYZ(1,1,1)
n1= modelSpace.newNodeXYZ(0,1,1)
n2= modelSpace.newNodeXYZ(1,0,1)
...
lp0.newNodalLoad(n0.tag,xc.Vector([-F,-F, -F, M, 2*M, 3*M]))
...
R10= n10.getReaction
errR10= math.sqrt(R10[0]**2+R10[1]**2+R10[2]**2+(R10[3]+M)**2+(R10[4]+2*M)**2+(R10[5]+3*M)**2)
R1= n1.getReaction
errR1= math.sqrt((R1[0]-F)**2+R1[1]**2+R1[2]**2+R1[3]**2+R1[4]**2+R1[5]**2)
R2= n2.getReaction
errR2= math.sqrt(R2[0]**2+(R2[1]-F)**2+R2[2]**2+R2[3]**2+R2[4]**2+R2[5]**2)
R3= n3.getReaction
errR3= math.sqrt(R3[0]**2+R3[1]**2+(R3[2]-F)**2+R3[3]**2+R3[4]**2+R3[5]**2)



ratio1= abs(abs(deltaX-refDeltaX)/refDeltaX)
ratio2= abs(abs(deltaY-refDeltaY)/refDeltaY)
ratio3= abs(abs(deltaY-refDeltaZ)/refDeltaZ)
ratio4= abs(abs(rotX-refRotX)/refRotX)
ratio5= abs(abs(rotY-refRotY)/refRotY)
ratio6= abs(abs(rotZ-refRotZ)/refRotZ)

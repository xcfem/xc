...
horizGM, horizGMSize= gmu.uniform_excitation_from_simple_record(modelSpace, name= "horizGM", dof= 0, inputFileName= horizAccelFilePath, dt= dt, cod_ts= 'horizAccel', factor= g*scale, vel0= 0.0)
modelSpace.addLoadCaseToDomain(horizGM.name)
vertGM, vertGMSize= gmu.uniform_excitation_from_simple_record(modelSpace, name= "vertGM", dof= 1, inputFileName= vertAccelFilePath, dt= dt, cod_ts= 'vertAccel', factor= g*scale, vel0= 0.0)
...
numberOfSteps= max(horizGMSize, vertGMSize)

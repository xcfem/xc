...
gammaD= -rough_calculations.masonry_bridge.calcGamma(f,j,k,r,xD)
hA= rough_calculations.masonry_bridge.yAxis(f,j,k,r,xA)
hC= rough_calculations.masonry_bridge.yAxis(f,j,k,r,xC)
hD= rough_calculations.masonry_bridge.yAxis(f,j,k,r,xD)
hB= rough_calculations.masonry_bridge.yAxis(f,j,k,r,xB)
LR= xB-xA
hHalfL= rough_calculations.masonry_bridge.yAxis(f,j,k,r,L/2)
...
# Loads
eta= v*(PPS*hS*LR+PPR*(hR*LR+rough_calculations.masonry_bridge.aux1(f,j,k,r,xB,xA)))
phi= v*(PPS*hS*a+PPR*(hR*a+rough_calculations.masonry_bridge.aux1(f,j,k,r,xC,xA)))
psi= v*(PPS*hS*b+PPR*(hR*b+rough_calculations.masonry_bridge.aux1(f,j,k,r,xB,xD)))
...
vQt= rough_calculations.masonry_bridge.vQtrans(v,delta,hRcle)
qtrans= Q/vQt
lQt= rough_calculations.masonry_bridge.lQtrans(a,delta,hRcle)
X= qtrans/lQt

# Résistance
Nadmis= -1.25e6 #Effort axial admisible
Madmis= rough_calculations.masonry_bridge.diagInteraction(Nadmis,d,v,alpha,beta)
E= rough_calculations.masonry_bridge.calcE6p27(X,qrep,L,LR,v,lQt,a,b,hA,hB,hC,hD,xA)
F= rough_calculations.masonry_bridge.calcF6p28(R,LR,a,b,eta,phiS,etaW,psiT,Madmis,Madmis,Madmis,RzB,RzD,hA,hB,hC,hD)
G= rough_calculations.masonry_bridge.calcG6p29(X,qrep,L,LR,v,lQt,a,b,hA,hB,hC,hD,xA,gammaD)
H= rough_calculations.masonry_bridge.calcH6p30(LR,a,eta,psi,phiS,etaW,Madmis,Madmis,Madmis,RzB,hA,hB,hC,hD,gammaD)
n= rough_calculations.masonry_bridge.calcn6p32(alpha,beta,d,v,E,F,G,H)

rough_calculations.masonry_bridge.printVouteResults(L, f, j, k, r, a,b,gammaD,hA,hB,hC,hD,LR,d,v,hR,hS,hHalfL,alpha,beta,PPR,PPS,eta,etaW,phi,phiS,psi,psiT,mp,Kp,mc,Kc,R, RzB, RzD,qtrans,X,Nadmis,Madmis,E,F,G,H,n)

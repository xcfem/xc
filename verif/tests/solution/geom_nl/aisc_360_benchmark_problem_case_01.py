...
# ElasticBeam2d with corotational formulation.
...
steelColumn.title= 'AISC 360-16 benchmark problem Case 1 (corotational formulation)'
...
p1= pointHandler.newPntFromPos3d(geom.Pos3d(0.0,height,0.0))
...
cooTrf= modelSpace.newCorotCrdTransf("corot")    
...
pLoad= xc.Vector([0.0,-667e3, 0.0])
p1.getNode().newLoad(pLoad)
  
## Point load 2.
cLC= loadCaseManager.setCurrentLoadCase('p2')
pLoad= xc.Vector([0.0,-1334e3,0.0])
p1.getNode().newLoad(pLoad)

## Point load 3.
cLC= loadCaseManager.setCurrentLoadCase('p3')
pLoad= xc.Vector([0.0,-2001e3, 0.0])
p1.getNode().newLoad(pLoad)

## Control sections
midHeight1= height/2.0
midPos1= geom.Pos3d(0.0,midHeight1,0.0)

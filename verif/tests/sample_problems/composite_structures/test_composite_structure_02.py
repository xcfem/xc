...
p0= modelSpace.newKPoint(-L1,0)
p1= modelSpace.newKPoint(0,0)
p2= modelSpace.newKPoint(L2,0)
p3= modelSpace.newKPoint(L2, websHeight)
p4= modelSpace.newKPoint(0, websHeight)
p5= modelSpace.newKPoint(-L1, websHeight)
## Create surfaces.
...
# Constraints.
modelSpace.fixNode("F0", p0.getNode().tag)
modelSpace.fixNode("00", p1.getNode().tag)
modelSpace.fixNode("F0", p2.getNode().tag)

# Locate offending elements.
offendingElementTags= [702, 703, 704, 736, 737, 738, 766, 767]
...
# Compute solution.
if(creepOnDeck):
    solProc= predefined_solutions.PlainKrylovNewton(prb= feProblem, convergenceTestTol= 1e-3, convTestType= 'energy_incr_conv_test', maxNumIter= 50, printFlag= 1)
else:
